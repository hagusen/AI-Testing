	...
	public class InputGetKey : AnyBlock {
		...
		protected override void OnExecute() {
			if(storeValue.isAssigned) {
				storeValue.Set(GetKey(keyCode.GetValue<KeyCode>()));
			}
		}

		protected override bool OnValidate() {
			bool condition = GetKey(keyCode.GetValue<KeyCode>());
			if(storeValue.isAssigned) {
		...
		public override string GenerateCode(Object obj) {
			string data = null;
			if(getKeyType == GetKeyType.GetKey) {
				data = CodeGenerator.GetInvokeCode(typeof(Input), "GetKey", CodeGenerator.ParseValue((object)keyCode)).ToString();
			} else if(getKeyType == GetKeyType.GetKeyDown) {
				data = CodeGenerator.GetInvokeCode(typeof(Input), "GetKeyDown", CodeGenerator.ParseValue((object)keyCode)).ToString();
			} else if(getKeyType == GetKeyType.GetKeyUp) {
				data = CodeGenerator.GetInvokeCode(typeof(Input), "GetKeyUp", CodeGenerator.ParseValue((object)keyCode)).ToString();
			}
			if(storeValue.isAssigned) {
