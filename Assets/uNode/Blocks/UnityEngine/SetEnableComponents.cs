	...
	public class SetEnableComponents : Action {
		...
		protected override void OnExecute() {
			foreach(var variable in components) {
				Component comp = variable.GetValue<Component>();
				if(comp != null) {
					Behaviour behavior = comp as Behaviour;
					if(behavior != null) {
						behavior.enabled = setValue.GetValue<bool>();
						continue;
					}
					Renderer rend = comp as Renderer;
					if(rend != null) {
						rend.enabled = setValue.GetValue<bool>();
						continue;
					}
					Collider col = comp as Collider;
					if(col != null) {
						col.enabled = setValue.GetValue<bool>();
						continue;
					}
				}
			}
		...
		public override string GenerateCode(Object obj) {
			...
			foreach(var var in components) {
				if(var.isAssigned) {
					data += CodeGenerator.GenerateSetCode(
						var.ParseValue().Access("enabled"),
						setValue.ParseValue()
					);
				}
			}
