...
namespace MaxyGames {
	...
	public static class SerializerUtility {
		public static object DeserializeMigration(ref OdinSerializedData odinSerializerData, ref string fullSerializerData, ref List<Object> fullSerializerReferences, System.Type fullSerializerType = null) {
			if(string.IsNullOrEmpty(fullSerializerData)) {
				return Deserialize(odinSerializerData);
			} else {
				object result = JsonHelper.Deserialize(fullSerializerData, fullSerializerType, fullSerializerReferences);
				odinSerializerData = SerializeValue(result);
				fullSerializerData = string.Empty;
				fullSerializerReferences?.Clear();
				return result;
			}
		}

		public static object DeserializeMigration(ref byte[] odinSerializerData, ref string fullSerializerData, ref List<Object> references, System.Type type = null) {
			if(string.IsNullOrEmpty(fullSerializerData)) {
				return Deserialize(odinSerializerData, references, type);
			} else {
				object result = JsonHelper.Deserialize(fullSerializerData, type, references);
				odinSerializerData = Serialize(result, out references);
				fullSerializerData = string.Empty;
				return result;
			}
		}
		...
		public static T Deserialize<T>(OdinSerializedData serializedData) {
			if(serializedData == null)
				return default;
			if(typeof(T).IsCastableTo(typeof(Object)) || serializedData.data.Length == 0) {
