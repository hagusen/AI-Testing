...
namespace MaxyGames.uNode {
	...
	public static class uNodeUtility {
		...
		#region Debug
		/// <summary>
		/// this will changed by uNodeEditorInitializer
		/// </summary>
		...
		/// </summary>
		public static void InvokeValueNode(object owner, int objectUID, int nodeUID, object nodeValue) {
		...
		/// Call this function to debug EventNode that using value node.
		/// </summary>
		public static void InvokeValueTransition(object owner, int objectUID, int nodeUID, int valueID, object value, bool isSet = false) {
			if(!useDebug || !Application.isPlaying)
				return;
			Dictionary<object, DebugData> debugMap = null;
			if(!debugData.TryGetValue(objectUID, out debugMap)) {
			...
			if(!data.valueTransitionDebug.TryGetValue(nodeUID, out map)) {
				map = new Dictionary<int, DebugData.ValueDebug>();
				data.valueTransitionDebug[nodeUID] = map;
			}
		...
		/// </summary>
		public static void InvokeFlowTransition(object owner, int objectUID, int nodeUID, int valueID) {
			if(!useDebug || !Application.isPlaying)
				return;
			Dictionary<object, DebugData> debugMap = null;
			if(!debugData.TryGetValue(objectUID, out debugMap)) {
		...
		/// Call this function to debug Flow node.
		/// </summary>
		public static void InvokeFlowTransition(object owner, int objectUID, int nodeUID, string flowName) {
			if(!useDebug || !Application.isPlaying)
				return;
			Dictionary<object, DebugData> debugMap = null;
			if(!debugData.TryGetValue(objectUID, out debugMap)) {
		...
		public static void InvokeNode(object owner, int objectUID, int nodeUID, bool? state) {
			InvokeNode(owner, objectUID, nodeUID, state == null ? StateType.Running : (state.Value ? StateType.Success : StateType.Failure));
		}
		...
		/// Call this function to debug the node.
		/// </summary>
		public static void InvokeNode(object owner, int objectUID, int nodeUID, StateType state) {
			if(!useDebug || !Application.isPlaying)
				return;
			Dictionary<object, DebugData> debugMap = null;
			if(!debugData.TryGetValue(objectUID, out debugMap)) {
		...
		/// </summary>
		public static void InvokeTransition(object owner, int objectUID, int transitionUID) {
			if(!useDebug || !Application.isPlaying)
				return;
			Dictionary<object, DebugData> debugMap = null;
			if(!debugData.TryGetValue(objectUID, out debugMap)) {
		...
		/// <param name="objectUID"></param>
		/// <param name="nodeUID"></param>
		/// <param name="transitionUID"></param>
		public static void InvokeNodeTransition(object owner, int objectUID, int nodeUID, int transitionUID) {
			if(!useDebug || !Application.isPlaying)
				return;
			Dictionary<object, DebugData> debugMap = null;
			if(!debugData.TryGetValue(objectUID, out debugMap)) {
			...
			Dictionary<int, float> map = null;
			if(!data.nodeTransitionDebug.TryGetValue(nodeUID, out map)) {
				map = new Dictionary<int, float>();
				data.nodeTransitionDebug[nodeUID] = map;
			}
			map[transitionUID] = Time.unscaledTime;
		}
...
#region GetDisplayName
		...
		public class ConnectionData {
			public GUIContent centerLabel;
			public bool toRight = false, fromRight = true;
			public bool isFlowConnection = true;
			...
			public Action onMouseInsideConnection;
		...
		public class DebugData {
			...
			/// </summary>
			public Dictionary<int, Dictionary<int, ValueDebug>> valueTransitionDebug = new Dictionary<int, Dictionary<int, ValueDebug>>();
			/// <summary>
			/// Data to debug EventNode transition with ValueNode
			/// </summary>
			public Dictionary<int, Dictionary<int, ValueDebug>> eventValueNodeDebug = new Dictionary<int, Dictionary<int, ValueDebug>>();
			/// <summary>
			...
			/// Data to debug Node Transition
			/// </summary>
			public Dictionary<int, Dictionary<int, float>> nodeTransitionDebug = new Dictionary<int, Dictionary<int, float>>();
		}

		public class TransitionData {
			/// <summary>
			/// The sender of the transition.
			/// </summary>
			public NodeComponent sender;
			/// <summary>
			...
			if(!mMember.target.isAssigned) {
				...
				if(mMember.target.targetType != MemberData.TargetType.Values) {
					if(mMember.target.startType == null) {
						RegisterEditorError(owner, "Missing type: " + mMember.target.startTypeName);
					}
				} else {
					if(mMember.target.type == null) {
						RegisterEditorError(owner, "Missing type: " + mMember.target.targetTypeName);
					}
				}
			}
			try {
				if(mMember.target.isAssigned && mMember.target.SerializedItems?.Length > 0) {
					...
					for(int z = 0; z < mMember.target.SerializedItems.Length; z++) {
						...
						Type[] paramsType = mMember.target.ParameterTypes[z];
