...
namespace MaxyGames {
	...
	public static class TypeSerializer {
		...
		public static Type Deserialize(string fullName, List<UnityEngine.Object> references, bool throwException = true) {
			...
			if(fullName == "[runtime]") {
				if(references.Count > 0) {
					return ReflectionUtils.GetRuntimeType(references[0]);
				}
			}
			//if(fullName[0].Equals('{')) {
			//	var member = JsonHelper.Deserialize(fullName, typeof(MemberData), reference) as MemberData;
			//	if(member != null && member.IsTargetingType) {
			//		return member.Get<Type>();
			//	}
			//}
			return Deserialize(fullName, throwException);
		}
					...
					for(int i = 0; i < data1[1].Length; i++) {
						...
						if(c == '[') {
							...
							if(deepLevel == 2) {
								...
								var cType = Deserialize(string.Join("", new string[] { new string(listChar.ToArray()) }), false);
