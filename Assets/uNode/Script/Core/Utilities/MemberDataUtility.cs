...
namespace MaxyGames {
	...
	public static class MemberDataUtility {
		...
		/// <param name="targets"></param>
		/// <returns></returns>
		public static string GetParameterName(TypeData genericData, IList<string> types = null, params UnityEngine.Object[] targets) {
			return GetParameterName(genericData, types, targets);
		}
		...
		/// <param name="GenericType"></param>
		/// <param name="ParameterType"></param>
		public static void DeserializeMemberItem(MemberData.ItemData item, IList<UnityEngine.Object> targets, out Type[] GenericType, out Type[] ParameterType) {
			Type[] GType = Type.EmptyTypes;
			Type[] PType = Type.EmptyTypes;
			if(item != null) {
				if(item.genericArguments != null) {
		...
		/// <param name="targets"></param>
		/// <returns></returns>
		public static System.Type GetParameterType(TypeData typeData, IList<Type> types = null, IList<UnityEngine.Object> targets = null) {
			int array = 0;
			...
			if(t.IsGenericTypeDefinition && typeData.parameters != null && typeData.parameters.Length > 0) {
				System.Type[] T = new System.Type[typeData.parameters.Length];
		...
		/// <param name="targets"></param>
		/// <returns></returns>
		public static System.Type GetParameterType(TypeData typeData, IList<GenericParameterData> types = null, IList<UnityEngine.Object> targets = null) {
			int array = 0;
			string n = typeData.name;
			while(n.EndsWith("[]")) {
				n = n.Remove(n.Length - 2);
		...
		/// <param name="targets"></param>
		/// <returns></returns>
		public static System.Type GetGenericType(TypeData genericData, IList<UnityEngine.Object> targets) {
			int array = 0;
			string n = genericData.name;
			while(n.EndsWith("[]")) {
				n = n.Remove(n.Length - 2);
			...
			} else if(genericData.name == RUNTIME_ID) {
				if(genericData.references?.Count > 0) {
			...
			if(t.IsGenericTypeDefinition && genericData.parameters != null && genericData.parameters.Length > 0) {
				System.Type[] T = new System.Type[genericData.parameters.Length];
				for(int i = 0; i < genericData.parameters.Length; i++) {
					T[i] = GetGenericType(genericData.parameters[i], targets);
				}
				t = t.MakeGenericType(T);
			}
			while(array > 0) {
		...
		/// <param name="genericData"></param>
		/// <returns></returns>
		public static System.Type GetGenericType(TypeData genericData) {
			int array = 0;
			string n = genericData.name;
			while(n.EndsWith("[]")) {
				n = n.Remove(n.Length - 2);
			...
			if(t.IsGenericTypeDefinition && genericData.parameters != null && genericData.parameters.Length > 0) {
				System.Type[] T = new System.Type[genericData.parameters.Length];
				for(int i = 0; i < genericData.parameters.Length; i++) {
					T[i] = GetGenericType(genericData.parameters[i]);
				}
				t = t.MakeGenericType(T);
			}
			while(array > 0) {
		...
		public static void UpdateMemberInstance(MemberData member, Type type) {
			...
			if((member.instance == null || !(member.instance is MemberData))) {
				if(ReflectionUtils.CanCreateInstance(type)) {
					if(member.instance != null && !(member.instance is MemberData)) {
						member.instance = new MemberData(member.instance, MemberData.TargetType.Values);
					} else {
						member.instance = new MemberData(ReflectionUtils.CreateInstance(type), MemberData.TargetType.Values);
					}
				} else {
					member.instance = new MemberData(member.instance);
				}
		...
		public static void UpdateMultipurposeMember(MultipurposeMember member) {
			...
			if(member.target.isTargeted) {
				...
				if(member.target.SerializedItems?.Length > 0) {
					if(member.target.ParameterTypes == null) return;
					int totalParam = 0;
							...
							for(int x = 0; x < paramsType.Length; x++) {
								...
								if(param == null || !param.isTargeted) {
									if(ReflectionUtils.CanCreateInstance(PType)) {
										member.parameters[totalParam] = new MemberData(ReflectionUtils.CreateInstance(PType));
									} else {
										member.parameters[totalParam] = MemberData.empty;
									}
								}
								totalParam++;
							}
