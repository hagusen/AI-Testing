...
namespace MaxyGames {
	...
	public class FilterAttribute : Attribute {
		/// <summary>
		/// Custom Filter a members.
		/// </summary>
		public Func<MemberInfo, bool> ValidateMember;
		/// <summary>
		...
		public bool IsValidType(Type t) {
			...
			if(!DisplayNativeType && !(t is uNode.RuntimeType)) return false;
			if(!DisplayInterfaceType && t.IsInterface) return false;
			if(DisplayInterfaceType && AllowInterface && t.IsInterface) return true;
			if(!DisplayReferenceType && !t.IsValueType && !t.IsInterface) return false;
			if(!DisplayValueType && t.IsValueType) return false;
			if(!DisplaySealedType && t.IsSealed) return false;
			if(!DisplayAbstractType && t.IsAbstract) return false;
			if(!DisplayRuntimeType && t is uNode.RuntimeType) return false;
			if(OnlyGetType && t.IsAbstract && t.IsSealed) return false;//Ensure the static type is not valid when only get type
			if(HideTypes != null && HideTypes.Count > 0) {
				for(int i = 0; i < HideTypes.Count; i++) {
					Type type = HideTypes[i];
					if(type == null) continue;
		...
		public string Tooltip {
			get {
				...
				if(Types != null && Types.Count > 0) {
					if(_tooltip == null) {
						_tooltip = string.Join("\n", Types.ConvertAll(t => t.FullName).ToArray());
					}
					return _tooltip;
				}
				return null;
