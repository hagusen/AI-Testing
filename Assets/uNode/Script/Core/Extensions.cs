...
namespace MaxyGames {
	public static class Extensions {
		...
		public static bool IsCastableTo(this Type from, Type to) {
			...
			var methods = from.FindImplicitOperator(to);
		...
		public static IEnumerable<MethodInfo> FindImplicitOperator(this Type from, Type to) {
			return from.GetMethods(BindingFlags.Public | BindingFlags.Static | BindingFlags.FlattenHierarchy).Where(m => m.ReturnType == to && (m.Name == "op_Implicit"));
		}
		...
		/// <param name="targetType"></param>
		/// <returns></returns>
		public static bool IsTargetingPinOrNode(this MemberData.TargetType targetType) {
				switch(targetType) {
					case MemberData.TargetType.FlowNode:
					case MemberData.TargetType.ValueNode:
		...
		public static string AddFunction(this string str, string function, params string[] paramValue) {
			if(!string.IsNullOrEmpty(str)) {
				return CodeGenerator.GenerateInvokeCode(str, function, paramValue).RemoveLast();
			}
			return str;
		}
