...
namespace MaxyGames.Serializer {
	...
	public static class Serializer {
		...
		public static SerializedObject SerializeObject(GameObject gameObject) {
			...
			serializedObject.data = gameObject.name;
		...
		public static string Serialize(object obj, bool Indented, SerializedData serializedData) {
			...
			s.AddConverter(new ObjectConverter() { data = serializedData });
		...
		public static void Deserialize<T>(string json, ref T instance, SerializedData serializedData) {
			try {
				...
				if(data != null) {
					...
					serializer.AddConverter(new ObjectConverter() { data = serializedData });
		...
		/// </summary>
		private class ObjectConverter : fsConverter {
			public bool isChild = false;
			public SerializedData data;

			static FullSerializer.Internal.fsReflectedConverter converter = new FullSerializer.Internal.fsReflectedConverter();

			private SerializedData actualReferece {
				get {
					return data;
				}
			}
			...
			public override fsResult TrySerialize(object instance, out fsData serialized, System.Type storageType) {
				if(!isChild) {
					...
					if(converter.CanProcess(storageType)) {
						return converter.TrySerialize(instance, out serialized, storageType);
					}
				}
				if(actualReferece != null) {
					var obj = instance as Object;
					if(actualReferece.serializedObjects.ContainsKey(obj.GetInstanceID())) {
						serialized = new fsData(obj.GetInstanceID().ToString());
					} else {
						actualReferece.objects.Add(instance as Object);
						if(instance != null) {
							serialized = new fsData(actualReferece.objects.Count - 1);
						} else {
			...
			public override fsResult TryDeserialize(fsData data, ref object instance, System.Type storageType) {
				if(!isChild) {
					...
					converter.Serializer = Serializer;
					if(converter.CanProcess(storageType)) {
						return converter.TryDeserialize(data, ref instance, storageType);
					}
				}
				if(actualReferece != null) {
					if(data.IsInt64) {
						int index = (int)data.AsInt64;
						if(actualReferece.objects.Count > index) {
							instance = actualReferece.objects[index];
						}
					} else if(data.IsString) {
						int id = int.Parse(data.AsString);
						if(this.data.deserializedObjects.ContainsKey(id)) {
							instance = this.data.deserializedObjects[id];
						}
					}
				}
				return fsResult.Success;
