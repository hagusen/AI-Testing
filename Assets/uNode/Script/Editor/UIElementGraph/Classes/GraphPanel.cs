	...
	internal class GraphPanel : VisualElement, IDragManager, IDisposable {
		...
		void InitializeView() {
			...
			{//Property
				...
				plusElement.onClick = () => {
					var mPos = (plusElement.clickedEvent.currentTarget as VisualElement).GetScreenMousePosition((plusElement.clickedEvent as IMouseEvent).localMousePosition, graph.window);
					ShowTypeMenu(mPos, type => {
						string fName = "newProperty";
						if (editorData.graph.Properties != null && editorData.graph.Properties.Count > 0) {
							int index = 0;
							while (true) {
								index++;
								bool found = false;
								foreach (var f in editorData.graph.Properties) {
								...
								if (found) {
									fName = "newProperty" + index;
								} else {
									break;
								}
							}
						}
						if (editorData.graph.Variables != null && editorData.graph.Variables.Count > 0) {
							int index = 0;
							while (true) {
								index++;
								bool found = false;
								foreach (var f in editorData.graph.Variables) {
									if (f != null && f.Name.Equals(fName)) {
										found = true;
										break;
									}
						...
						NodeEditorUtility.AddNewProperty(editorData.graph, fName, delegate (uNodeProperty prop) {
			...
			{//Function
				...
				plusElement.onClick = () => {
					...
					plusElement.menu.AppendAction("Add new", act => {
						string fName = "NewFunction";
						if (editorData.graph.Functions.Count > 0) {
							int index = 0;
							while (true) {
								index++;
								bool found = false;
								foreach (var f in editorData.graph.Functions) {
									if (f != null && f.Name.Equals(fName)) {
										found = true;
										break;
									}
								...
								if (found) {
									fName = "NewFunction" + index;
								} else {
									break;
								}
							}
						}
						NodeEditorUtility.AddNewFunction(editorData.graph, fName, typeof(void));
						graph.Refresh();
					});
					if (editorData.graph is uNodeRuntime || editorData.graph is uNodeClass) {
						plusElement.menu.AppendAction("Add new coroutine", act => {
							...
							NodeEditorUtility.AddNewFunction(editorData.graph, fName, typeof(System.Collections.IEnumerator));
							graph.Refresh();
						});
					}
					...
					if (typeof(MonoBehaviour).IsAssignableFrom(inheritType)) {
						...
						{//Awake Event
							...
							plusElement.menu.AppendAction("UnityEvent/Awake()", act => {
								if (!hasFunction) {
									NodeEditorUtility.AddNewFunction(editorData.graph, "Awake", typeof(void));
									graph.Refresh();
								}
						...
						{//Start Event
							...
							plusElement.menu.AppendAction("UnityEvent/Start()", act => {
								if (!hasFunction) {
									NodeEditorUtility.AddNewFunction(editorData.graph, "Start", typeof(void));
									graph.Refresh();
								}
							}, a => hasFunction ? DropdownMenuAction.Status.Checked : DropdownMenuAction.Status.Normal);
						}
						{//Update Event
							...
							plusElement.menu.AppendAction("UnityEvent/Update()", act => {
								if (!hasFunction) {
									NodeEditorUtility.AddNewFunction(editorData.graph, "Update", typeof(void));
									graph.Refresh();
								}
							}, a => hasFunction ? DropdownMenuAction.Status.Checked : DropdownMenuAction.Status.Normal);
						}
						{//FixedUpdate Event
							...
							plusElement.menu.AppendAction("UnityEvent/FixedUpdate()", act => {
								if (!hasFunction) {
									NodeEditorUtility.AddNewFunction(editorData.graph, "FixedUpdate", typeof(void));
									graph.Refresh();
								}
							}, a => hasFunction ? DropdownMenuAction.Status.Checked : DropdownMenuAction.Status.Normal);
						}
						{//LateUpdate Event
							...
							plusElement.menu.AppendAction("UnityEvent/LateUpdate()", act => {
								if (!hasFunction) {
									NodeEditorUtility.AddNewFunction(editorData.graph, "LateUpdate", typeof(void));
									graph.Refresh();
								}
							}, a => hasFunction ? DropdownMenuAction.Status.Checked : DropdownMenuAction.Status.Normal);
						}
						{//OnAnimatorIK Event
							...
							plusElement.menu.AppendAction("UnityEvent/OnAnimatorIK(int)", act => {
								if (!hasFunction) {
									NodeEditorUtility.AddNewFunction(editorData.graph, "OnAnimatorIK", typeof(void), new string[] { "layerIndex" }, new Type[] { typeof(int) });
									graph.Refresh();
								}
							}, a => hasFunction ? DropdownMenuAction.Status.Checked : DropdownMenuAction.Status.Normal);
						}
						{//OnAnimatorMove Event
							...
							plusElement.menu.AppendAction("UnityEvent/OnAnimatorMove()", act => {
								if (!hasFunction) {
									NodeEditorUtility.AddNewFunction(editorData.graph, "OnAnimatorMove", typeof(void));
									graph.Refresh();
								}
							}, a => hasFunction ? DropdownMenuAction.Status.Checked : DropdownMenuAction.Status.Normal);
						}
						{//OnApplicationFocus Event
							...
							plusElement.menu.AppendAction("UnityEvent/OnApplicationFocus(bool)", act => {
								if (!hasFunction) {
									NodeEditorUtility.AddNewFunction(editorData.graph, "OnApplicationFocus", typeof(void), new string[] { "focusStatus" }, new Type[] { typeof(bool) });
									graph.Refresh();
								}
							}, a => hasFunction ? DropdownMenuAction.Status.Checked : DropdownMenuAction.Status.Normal);
						}
						{//OnApplicationPause Event
							...
							plusElement.menu.AppendAction("UnityEvent/OnApplicationPause(bool)", act => {
								if (!hasFunction) {
									NodeEditorUtility.AddNewFunction(editorData.graph, "OnApplicationPause", typeof(void), new string[] { "pauseStatus" }, new Type[] { typeof(bool) });
									graph.Refresh();
								}
							}, a => hasFunction ? DropdownMenuAction.Status.Checked : DropdownMenuAction.Status.Normal);
						}
						{//OnApplicationQuit Event
							...
							plusElement.menu.AppendAction("UnityEvent/OnApplicationQuit()", act => {
								if (!hasFunction) {
									NodeEditorUtility.AddNewFunction(editorData.graph, "OnApplicationQuit", typeof(void));
									graph.Refresh();
								}
							}, a => hasFunction ? DropdownMenuAction.Status.Checked : DropdownMenuAction.Status.Normal);
						}
						{//OnBecameInvisible Event
							...
							plusElement.menu.AppendAction("UnityEvent/OnBecameInvisible()", act => {
								if (!hasFunction) {
									NodeEditorUtility.AddNewFunction(editorData.graph, "OnBecameInvisible", typeof(void));
									graph.Refresh();
								}
							}, a => hasFunction ? DropdownMenuAction.Status.Checked : DropdownMenuAction.Status.Normal);
						}
						{//OnBecameVisible Event
							...
							plusElement.menu.AppendAction("UnityEvent/OnBecameVisible()", act => {
								if (!hasFunction) {
									NodeEditorUtility.AddNewFunction(editorData.graph, "OnBecameVisible", typeof(void));
									graph.Refresh();
								}
							}, a => hasFunction ? DropdownMenuAction.Status.Checked : DropdownMenuAction.Status.Normal);
						}
						{//OnCollisionEnter Event
							...
							plusElement.menu.AppendAction("UnityEvent/OnCollisionEnter(Collision)", act => {
								if (!hasFunction) {
									NodeEditorUtility.AddNewFunction(editorData.graph, "OnCollisionEnter", typeof(void), new string[] { "collisionInfo" }, new Type[] { typeof(Collision) });
									graph.Refresh();
								}
							}, a => hasFunction ? DropdownMenuAction.Status.Checked : DropdownMenuAction.Status.Normal);
						}
						{//OnCollisionEnter2D Event
							...
							plusElement.menu.AppendAction("UnityEvent/OnCollisionEnter2D(Collision2D)", act => {
								if (!hasFunction) {
									NodeEditorUtility.AddNewFunction(editorData.graph, "OnCollisionEnter2D", typeof(void), new string[] { "collisionInfo" }, new Type[] { typeof(Collision2D) });
									graph.Refresh();
								}
							}, a => hasFunction ? DropdownMenuAction.Status.Checked : DropdownMenuAction.Status.Normal);
						}
						{//OnCollisionExit Event
							...
							plusElement.menu.AppendAction("UnityEvent/OnCollisionExit(Collision)", act => {
								if (!hasFunction) {
									NodeEditorUtility.AddNewFunction(editorData.graph, "OnCollisionExit", typeof(void), new string[] { "collisionInfo" }, new Type[] { typeof(Collision) });
									graph.Refresh();
								}
							}, a => hasFunction ? DropdownMenuAction.Status.Checked : DropdownMenuAction.Status.Normal);
						}
						{//OnCollisionExit2D Event
							...
							plusElement.menu.AppendAction("UnityEvent/OnCollisionExit2D(Collision2D)", act => {
								if (!hasFunction) {
									NodeEditorUtility.AddNewFunction(editorData.graph, "OnCollisionExit2D", typeof(void), new string[] { "collisionInfo" }, new Type[] { typeof(Collision2D) });
									graph.Refresh();
								}
							}, a => hasFunction ? DropdownMenuAction.Status.Checked : DropdownMenuAction.Status.Normal);
						}
						{//OnCollisionStay Event
							...
							plusElement.menu.AppendAction("UnityEvent/OnCollisionStay(Collision)", act => {
								if (!hasFunction) {
									NodeEditorUtility.AddNewFunction(editorData.graph, "OnCollisionStay", typeof(void), new string[] { "collisionInfo" }, new Type[] { typeof(Collision) });
									graph.Refresh();
								}
							}, a => hasFunction ? DropdownMenuAction.Status.Checked : DropdownMenuAction.Status.Normal);
						}
						{//OnCollisionStay2D Event
							...
							plusElement.menu.AppendAction("UnityEvent/OnCollisionStay2D(Collision2D)", act => {
								if (!hasFunction) {
									NodeEditorUtility.AddNewFunction(editorData.graph, "OnCollisionStay2D", typeof(void), new string[] { "collisionInfo" }, new Type[] { typeof(Collision2D) });
									graph.Refresh();
								}
							}, a => hasFunction ? DropdownMenuAction.Status.Checked : DropdownMenuAction.Status.Normal);
						}
						{//OnDestroy Event
							...
							plusElement.menu.AppendAction("UnityEvent/OnDestroy()", act => {
								if (!hasFunction) {
									NodeEditorUtility.AddNewFunction(editorData.graph, "OnDestroy", typeof(void));
									graph.Refresh();
								}
							}, a => hasFunction ? DropdownMenuAction.Status.Checked : DropdownMenuAction.Status.Normal);
						}
						{//OnDisable Event
							...
							plusElement.menu.AppendAction("UnityEvent/OnDisable()", act => {
								if (!hasFunction) {
									NodeEditorUtility.AddNewFunction(editorData.graph, "OnDisable", typeof(void));
									graph.Refresh();
								}
							}, a => hasFunction ? DropdownMenuAction.Status.Checked : DropdownMenuAction.Status.Normal);
						}
						{//OnEnable Event
							...
							plusElement.menu.AppendAction("UnityEvent/OnEnable()", act => {
								if (!hasFunction) {
									NodeEditorUtility.AddNewFunction(editorData.graph, "OnEnable", typeof(void));
									graph.Refresh();
								}
							}, a => hasFunction ? DropdownMenuAction.Status.Checked : DropdownMenuAction.Status.Normal);
						}
						{//OnGUI Event
							...
							plusElement.menu.AppendAction("UnityEvent/OnGUI()", act => {
								if (!hasFunction) {
									NodeEditorUtility.AddNewFunction(editorData.graph, "OnGUI", typeof(void));
									graph.Refresh();
								}
							}, a => hasFunction ? DropdownMenuAction.Status.Checked : DropdownMenuAction.Status.Normal);
						}
						{//OnMouseDown Event
							...
							plusElement.menu.AppendAction("UnityEvent/OnMouseDown()", act => {
								if (!hasFunction) {
									NodeEditorUtility.AddNewFunction(editorData.graph, "OnMouseDown", typeof(void));
									graph.Refresh();
								}
							}, a => hasFunction ? DropdownMenuAction.Status.Checked : DropdownMenuAction.Status.Normal);
						}
						{//OnMouseDrag Event
							...
							plusElement.menu.AppendAction("UnityEvent/OnMouseDrag()", act => {
								if (!hasFunction) {
									NodeEditorUtility.AddNewFunction(editorData.graph, "OnMouseDrag", typeof(void));
									graph.Refresh();
								}
							}, a => hasFunction ? DropdownMenuAction.Status.Checked : DropdownMenuAction.Status.Normal);
						}
						{//OnMouseEnter Event
							...
							plusElement.menu.AppendAction("UnityEvent/OnMouseEnter()", act => {
								if (!hasFunction) {
									NodeEditorUtility.AddNewFunction(editorData.graph, "OnMouseEnter", typeof(void));
									graph.Refresh();
								}
							}, a => hasFunction ? DropdownMenuAction.Status.Checked : DropdownMenuAction.Status.Normal);
						}
						{//OnMouseExit Event
							...
							plusElement.menu.AppendAction("UnityEvent/OnMouseExit()", act => {
								if (!hasFunction) {
									NodeEditorUtility.AddNewFunction(editorData.graph, "OnMouseExit", typeof(void));
									graph.Refresh();
								}
							}, a => hasFunction ? DropdownMenuAction.Status.Checked : DropdownMenuAction.Status.Normal);
						}
						{//OnMouseOver Event
							...
							plusElement.menu.AppendAction("UnityEvent/OnMouseOver()", act => {
								if (!hasFunction) {
									NodeEditorUtility.AddNewFunction(editorData.graph, "OnMouseOver", typeof(void));
									graph.Refresh();
								}
							}, a => hasFunction ? DropdownMenuAction.Status.Checked : DropdownMenuAction.Status.Normal);
						}
						{//OnMouseUp Event
							...
							plusElement.menu.AppendAction("UnityEvent/OnMouseUp()", act => {
								if (!hasFunction) {
									NodeEditorUtility.AddNewFunction(editorData.graph, "OnMouseUp", typeof(void));
									graph.Refresh();
								}
							}, a => hasFunction ? DropdownMenuAction.Status.Checked : DropdownMenuAction.Status.Normal);
						}
						{//OnMouseUpAsButton Event
							...
							plusElement.menu.AppendAction("UnityEvent/OnMouseUpAsButton()", act => {
								if (!hasFunction) {
									NodeEditorUtility.AddNewFunction(editorData.graph, "OnMouseUpAsButton", typeof(void));
									graph.Refresh();
								}
							}, a => hasFunction ? DropdownMenuAction.Status.Checked : DropdownMenuAction.Status.Normal);
						}
						{//OnPostRender Event
							...
							plusElement.menu.AppendAction("UnityEvent/OnPostRender()", act => {
								if (!hasFunction) {
									NodeEditorUtility.AddNewFunction(editorData.graph, "OnPostRender", typeof(void));
									graph.Refresh();
								}
							}, a => hasFunction ? DropdownMenuAction.Status.Checked : DropdownMenuAction.Status.Normal);
						}
						{//OnPreCull Event
							...
							plusElement.menu.AppendAction("UnityEvent/OnPreCull()", act => {
								if (!hasFunction) {
									NodeEditorUtility.AddNewFunction(editorData.graph, "OnPreCull", typeof(void));
									graph.Refresh();
								}
							}, a => hasFunction ? DropdownMenuAction.Status.Checked : DropdownMenuAction.Status.Normal);
						}
						{//OnPreRender Event
							...
							plusElement.menu.AppendAction("UnityEvent/OnPreRender()", act => {
								if (!hasFunction) {
									NodeEditorUtility.AddNewFunction(editorData.graph, "OnPreRender", typeof(void));
									graph.Refresh();
								}
							}, a => hasFunction ? DropdownMenuAction.Status.Checked : DropdownMenuAction.Status.Normal);
						}
						{//OnRenderObject Event
							...
							plusElement.menu.AppendAction("UnityEvent/OnRenderObject()", act => {
								if (!hasFunction) {
									NodeEditorUtility.AddNewFunction(editorData.graph, "OnRenderObject", typeof(void));
									graph.Refresh();
								}
							}, a => hasFunction ? DropdownMenuAction.Status.Checked : DropdownMenuAction.Status.Normal);
						}
						{//OnTransformChildrenChanged Event
							...
							plusElement.menu.AppendAction("UnityEvent/OnTransformChildrenChanged()", act => {
								if (!hasFunction) {
									NodeEditorUtility.AddNewFunction(editorData.graph, "OnTransformChildrenChanged", typeof(void));
									graph.Refresh();
								}
							}, a => hasFunction ? DropdownMenuAction.Status.Checked : DropdownMenuAction.Status.Normal);
						}
						{//OnTransformParentChanged Event
							...
							plusElement.menu.AppendAction("UnityEvent/OnTransformParentChanged()", act => {
								if (!hasFunction) {
									NodeEditorUtility.AddNewFunction(editorData.graph, "OnTransformParentChanged", typeof(void));
									graph.Refresh();
								}
							}, a => hasFunction ? DropdownMenuAction.Status.Checked : DropdownMenuAction.Status.Normal);
						}
						{//OnTriggerEnter Event
							...
							plusElement.menu.AppendAction("UnityEvent/OnTriggerEnter(Collider)", act => {
								if (!hasFunction) {
									NodeEditorUtility.AddNewFunction(editorData.graph, "OnTriggerEnter", typeof(void), new string[] { "colliderInfo" }, new Type[] { typeof(Collider) });
									graph.Refresh();
								}
							}, a => hasFunction ? DropdownMenuAction.Status.Checked : DropdownMenuAction.Status.Normal);
						}
						{//OnTriggerEnter2D Event
							...
							plusElement.menu.AppendAction("UnityEvent/OnTriggerEnter2D(Collider2D)", act => {
								if (!hasFunction) {
									NodeEditorUtility.AddNewFunction(editorData.graph, "OnTriggerEnter2D", typeof(void), new string[] { "colliderInfo" }, new Type[] { typeof(Collider2D) });
									graph.Refresh();
								}
							}, a => hasFunction ? DropdownMenuAction.Status.Checked : DropdownMenuAction.Status.Normal);
						}
						{//OnTriggerExit Event
							...
							plusElement.menu.AppendAction("UnityEvent/OnTriggerExit(Collider)", act => {
								if (!hasFunction) {
									NodeEditorUtility.AddNewFunction(editorData.graph, "OnTriggerExit", typeof(void), new string[] { "colliderInfo" }, new Type[] { typeof(Collider) });
									graph.Refresh();
								}
							}, a => hasFunction ? DropdownMenuAction.Status.Checked : DropdownMenuAction.Status.Normal);
						}
						{//OnTriggerExit2D Event
							...
							plusElement.menu.AppendAction("UnityEvent/OnTriggerExit2D(Collider2D)", act => {
								if (!hasFunction) {
									NodeEditorUtility.AddNewFunction(editorData.graph, "OnTriggerExit2D", typeof(void), new string[] { "colliderInfo" }, new Type[] { typeof(Collider2D) });
									graph.Refresh();
								}
							}, a => hasFunction ? DropdownMenuAction.Status.Checked : DropdownMenuAction.Status.Normal);
						}
						{//OnTriggerStay Event
							...
							plusElement.menu.AppendAction("UnityEvent/OnTriggerStay(Collider)", act => {
								if (!hasFunction) {
									NodeEditorUtility.AddNewFunction(editorData.graph, "OnTriggerStay", typeof(void), new string[] { "colliderInfo" }, new Type[] { typeof(Collider) });
									graph.Refresh();
								}
							}, a => hasFunction ? DropdownMenuAction.Status.Checked : DropdownMenuAction.Status.Normal);
						}
						{//OnTriggerStay2D Event
							...
							plusElement.menu.AppendAction("UnityEvent/OnTriggerStay2D(Collider2D)", act => {
								if (!hasFunction) {
									NodeEditorUtility.AddNewFunction(editorData.graph, "OnTriggerStay2D", typeof(void), new string[] { "colliderInfo" }, new Type[] { typeof(Collider2D) });
									graph.Refresh();
								}
							}, a => hasFunction ? DropdownMenuAction.Status.Checked : DropdownMenuAction.Status.Normal);
						}
						{//OnWillRenderObject Event
							...
							plusElement.menu.AppendAction("UnityEvent/OnWillRenderObject()", act => {
								if (!hasFunction) {
									NodeEditorUtility.AddNewFunction(editorData.graph, "OnWillRenderObject", typeof(void));
									graph.Refresh();
								}
							}, a => hasFunction ? DropdownMenuAction.Status.Checked : DropdownMenuAction.Status.Normal);
						}
			...
			{//Nested Types
				...
				plusElement.onClick = () => {
					plusElement.menu = new DropdownMenu();
					CreateNestedTypesMenu(plusElement.menu);
				};
				icon.parent.Add(plusElement);
				var toggle = header.Q("expanded") as Foldout;
				toggle.RegisterValueChangedCallback(evt => {
		...
		void ReloadView() {
					...
					if(showClasses) {
						for(int i=0;i< classes.Length;i++) {
							...
							if(root != null) {
								...
								if(i == 0 && displayName.StartsWith("_")) {
									displayName = root.gameObject.name;
								}
								Texture icon = uNodeEditorUtility.GetIcon(root.GetInheritType());
								...
								content.AddManipulator(new ContextualMenuManipulator(evt => {
									...
									evt.menu.AppendAction("Export to json", act => {
										var json = uNodeHelper.ExportGraphToJSON(current);
										string path = EditorUtility.SaveFilePanel("Export to json",
											"",
											current.gameObject.name + ".json",
											"json");
										if (path.Length != 0) {
											using (System.IO.FileStream stream = System.IO.File.Open(path, System.IO.FileMode.OpenOrCreate, System.IO.FileAccess.Write)) {
									...
									if (classes.Length > 1) {
										evt.menu.AppendAction("Remove", act => {
											if(uNodeEditorUtility.IsPrefabInstance(current)) {
												uNodeEditorUtility.UnlockPrefabInstance(current.gameObject);
											}
											Undo.RegisterCompleteObjectUndo(current.gameObject, "");
											if (current.RootObject != null) {
											...
											for (int x = 0; x < classes.Length; x++) {
												if (classes[x] != null) {
													uNodeEditor.ChangeTarget(classes[x]);
									...
									if (root is INestedClassSystem) {
										...
										evt.menu.AppendSeparator("");
										CreateNestedTypesMenu(evt.menu);
									}
								}));
					...
					foreach(var property in editorData.graph.Properties) {
						...
						content.AddManipulator(new ContextualMenuManipulator(evt => {
										...
										delegate (GameObject go) {
											...
											NodeEditorUtility.AddNewObject<Nodes.NodeAction>(editorData.graph, "Entry", func.transform, (node) => {
												...
												NodeEditorUtility.AddNewObject<NodeReturn>(editorData.graph, "return", func.transform, (rRode) => {
													node.onFinished = MemberData.CreateConnection(rRode, true);
							...
							evt.menu.AppendAction("Move Up", act => {
								...
								if(valid) {
									if(index != 0) {
										(current).transform.SetSiblingIndex(editorData.graph.Properties[index - 1].transform.GetSiblingIndex());
									}
								}
								graph.Refresh();
							});
					...
					foreach(var function in editorData.graph.Functions) {
						...
						content.AddManipulator(new ContextualMenuManipulator((evt) => {
							...
							evt.menu.AppendAction("Move Up", (act) => {
								...
								if(valid) {
									if(index != 0) {
										current.transform.SetSiblingIndex(editorData.graph.Functions[index - 1].transform.GetSiblingIndex());
									}
								}
								graph.Refresh();
							});
			...
			if(editorData.graph is IStateGraph stateGraph && stateGraph.canCreateGraph) {
				...
				if(showEvents && stateGraph.eventNodes?.Count > 0 && editorData.currentCanvas == editorData.graph) {
					foreach(var node in stateGraph.eventNodes) {
						...
						if(node.eventType != EventNode.EventType.Custom) {
							title = ObjectNames.NicifyVariableName(node.eventType.ToString());
						}
						...
						content.AddManipulator(new ContextualMenuManipulator((evt) => {
							...
							evt.menu.AppendAction("Remove", (act) => {
								if(editorData.graph)
									uNodeEditorUtility.RegisterUndo(editorData.graph, "Remove Node: " + current.Name);
					...
					foreach(var ctor in editorData.graph.Constuctors) {
						...
						content.AddManipulator(new ContextualMenuManipulator((evt) => {
							...
							evt.menu.AppendAction("Move Up", (act) => {
								...
								if(valid) {
									if(index != 0) {
										current.transform.SetSiblingIndex(editorData.graph.Constuctors[index - 1].transform.GetSiblingIndex());
									}
								}
								graph.Refresh();
							});
			...
			if(editorData.graph && editorData.graph is INestedClassSystem) {
				...
				uNodeRoot[] types = nestedTypes?.GetComponents<uNodeRoot>();
				if(showInterfaces && nestedTypes && types.Length > 0) {
		...
		#region Other
		private void CreateNestedTypesMenu(DropdownMenu menu) {
			var nestedSystem = editorData.graph as INestedClassSystem;
			var nestedTypes = nestedSystem.NestedClass;
