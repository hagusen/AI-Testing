...
namespace MaxyGames.uNode.Editors {
	public abstract class UNodeView : NodeView {
		...
		public PortView AddInputValuePort(PortData portData) {
			return AddPort(Orientation.Horizontal, Direction.Input, portData);
		}

		public PortView AddOutputValuePort(string fieldName, Func<Type> type, string portName = null) {
			FieldInfo field = targetNode.GetType().GetField(fieldName);
			return AddPort(Orientation.Horizontal, Direction.Output, new PortData() {
				portID = fieldName,
				getPortName = () => portName ?? ObjectNames.NicifyVariableName(field.Name),
				getPortValue = () => field.GetValueOptimized(targetNode),
				getPortType = type,
		...
		public PortView AddInputFlowPort(PortData portData) {
			...
			return AddPort(Orientation.Vertical, Direction.Input, portData);
		}
		...
		public PortView AddOutputFlowPort(Func<string> portName, Func<object> portValue, Action<object> onValueChanged) {
			return AddPort(Orientation.Vertical, Direction.Output, new PortData() {
		...
		public PortView AddOutputFlowPort(string fieldName, string portName = null) {
			FieldInfo field = targetNode.GetType().GetField(fieldName);
			return AddPort(Orientation.Vertical, Direction.Output,
				new PortData() {
					portID = fieldName,
					getPortName = () => portName ?? ObjectNames.NicifyVariableName(field.Name),
					getPortValue = () => field.GetValueOptimized(targetNode),
