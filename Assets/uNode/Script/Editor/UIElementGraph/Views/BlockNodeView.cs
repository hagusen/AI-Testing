			...
			if(block.blockType != BlockType.Condition) {
				...
				if(variable.Type == typeof(bool)) {
					menu.AddItem(new GUIContent("Compare Return Value"), false, () => {
						var act = BlockUtility.onAddCompareValue(new MemberData(variable, owner));
						node.RegisterUndo("");
						block.blocks.InsertBlock(blockIndex, act);
						node.MarkRepaint();
					});
		...
		private void DragHandleProperty(int blockIndex, uNodeProperty property) {
			...
			if(block.blockType != BlockType.Condition) {
				...
				if(property.ReturnType() == typeof(bool)) {
					menu.AddItem(new GUIContent("Compare Return Value"), false, () => {
						var act = BlockUtility.onAddCompareValue(new MemberData(property));
						node.RegisterUndo("");
						block.blocks.InsertBlock(blockIndex, act);
						node.MarkRepaint();
					});
		...
		private void DragHandleFunction(int blockIndex, uNodeFunction function) {
			...
			if(block.blockType != BlockType.Condition) {
				...
				if(function.ReturnType() == typeof(bool)) {
					menu.AddItem(new GUIContent("Compare Return Value"), false, () => {
						var act = BlockUtility.onAddCompareValue(MemberData.CreateFromValue(function));
						node.RegisterUndo("");
						block.blocks.InsertBlock(blockIndex, act);
						node.MarkRepaint();
					});
		...
		private void DragHandleMember(int blockIndex, FieldInfo member) {
			...
			if(block.blockType != BlockType.Condition) {
				...
				if(member.FieldType == typeof(bool)) {
					menu.AddItem(new GUIContent("Compare Return Value"), false, () => {
						var act = BlockUtility.onAddCompareValue(new MemberData(member));
						node.RegisterUndo("");
						block.blocks.InsertBlock(blockIndex, act);
						node.MarkRepaint();
					});
