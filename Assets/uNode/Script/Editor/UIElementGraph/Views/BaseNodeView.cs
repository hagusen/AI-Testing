...
namespace MaxyGames.uNode.Editors {
	public class BaseNodeView : UNodeView {
		...
		protected virtual void InitializeView() {
			...
			var node = targetNode as Node;
			var fields = NodeEditorUtility.GetFieldNodes(node.GetType());
			if (fields.Length > 0) {
				try {
					...
					for (int i = 0; i < fields.Length; i++) {
						...
						if (field.attribute is FieldConnectionAttribute) {
							...
							if (field.field.FieldType == typeof(MemberData)) {
								...
								if (FCA.isFlowConnection) {//Flow output
									float pos = 0;
										...
										() => {
											AddOutputFlowPort(
												new PortData() {
													...
													getPortValue = () => field.field.GetValueOptimized(node),
													getPortTooltip = () => {
								...
								if (obj is MultipurposeMember) {
									AddMultipurposeMemberPort(obj as MultipurposeMember, field, node);
								} else {
									throw new System.Exception("Can't draw connection for field: " + field.field.Name + "\nFieldType not supported.");
								}
							}
						} else if (field.attribute is OutputAttribute) {//Draw label on right.
							var FVA = field.attribute as OutputAttribute;
							if (FVA.type == null) {
							...
							AddInputFlowPort(
								new PortData() {
									portID = field.field.Name,
									getPortName = () => name,
									getConnection = () => MemberData.CreateConnection(targetNode as Node, field.field.Name),
									getPortTooltip = () => {
										var tooltip = ReflectionUtils.GetAttribute<TooltipAttribute>(field.attributes);
										return tooltip != null ? tooltip.tooltip : string.Empty;
									},
		...
		public override void ReloadView() {
			...
			if (debugView == null && targetNode is Node &&
				(Application.isPlaying && nodeFlowPort != null || uNodeUtility.HasBreakpoint(uNodeUtility.GetObjectID(targetNode)))) {
				debugView = new VisualElement() {
					name = "debug-container"
				};
				debugStateView = new VisualElement() {
					name = "debug-state"
				};
				titleButtonContainer.Add(debugView);
				debugView.Add(debugStateView);
			}
			if (debugView != null) {
				this.ExecuteAndScheduleAction(() => {
					...
					bool hasBreakpoint = uNodeUtility.HasBreakpoint(uNodeUtility.GetObjectID(targetNode));
					bool flag = Application.isPlaying && nodeFlowPort != null;
					uNodeUtility.DebugData.NodeDebug nodeDebug = null;
					var debugData = owner.graph.GetDebugInfo();
					if (flag && debugData != null && debugData.nodeDebug.ContainsKey(uNodeUtility.GetObjectID(node))) {
						nodeDebug = debugData.nodeDebug[uNodeUtility.GetObjectID(node)];
						flag = nodeDebug != null;
					} else {
						flag = false;
					}
					if (hasBreakpoint || flag) {
						if (flag) {
							debugStateView.style.SetBorderColor(Color.black);
							switch (nodeDebug.nodeState) {
								case StateType.Success:
									debugStateView.style.backgroundColor = UIElementUtility.Theme.nodeSuccessColor;
									break;
								case StateType.Failure:
									debugStateView.style.backgroundColor = UIElementUtility.Theme.nodeFailureColor;
									break;
								case StateType.Running:
									debugStateView.style.backgroundColor = UIElementUtility.Theme.nodeRunningColor;
									break;
							}
						}
						if (hasBreakpoint) {
							...
							debugView.style.backgroundColor = Color.clear;
							debugView.style.width = debugStateView.style.width;
							debugView.style.height = debugStateView.style.height;
						}
			...
			if (border != null) {
				...
				int flowInputCount = inputPorts.Count((p) => p.orientation == Orientation.Vertical);
				int flowOutputCount = outputPorts.Count((p) => p.orientation == Orientation.Vertical);
				if (flowInputCount + flowOutputCount > 0) {
					bool flag = outputPorts.Count((p) => p.orientation == Orientation.Vertical && !string.IsNullOrEmpty(p.GetName())) == 0;
		...
		protected void AddMultipurposeMemberPort(MultipurposeMember member, NodeEditorUtility.FieldNodeData field, NodeComponent node) {
			if (member != null && member.target != null) {
				...
				if (member.parameters != null && member.parameters.Length > 0 && member.target.SerializedItems?.Length > 0) {
					...
					for (int z = 0; z < member.target.SerializedItems.Length; z++) {
						if (members == null) {
							System.Type[] paramsType = member.target.ParameterTypes[z];
								...
								for (int x = 0; x < paramsType.Length; x++) {
									...
									if (mParam == null) {
										if (ReflectionUtils.CanCreateInstance(PType)) {
											mParam = new MemberData(ReflectionUtils.CreateInstance(PType));
										} else {
		...
		public override void SetPosition(Rect newPos) {
			...
			if ((xPos != 0 || yPos != 0) && owner.selection.Count == 1 && owner.selection.Contains(this) && owner.currentEvent != null) {
