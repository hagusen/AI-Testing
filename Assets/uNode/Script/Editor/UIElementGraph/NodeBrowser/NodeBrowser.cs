			...
			if(!memberMaps.TryGetValue(type, out members)) {
				...
				members.Sort((x, y) => {
					if(x is ConstructorInfo && y is ConstructorInfo) {
						return string.Compare((x as ConstructorInfo).GetParameters().Length.ToString(), (y as ConstructorInfo).GetParameters().Length.ToString());
					} else if(x is ConstructorInfo) {
						return -1;
					} else if(y is ConstructorInfo) {
						return 1;
					} else if(x is MethodInfo && y is MethodInfo) {
						int i = string.Compare(x.Name, y.Name);
						if(i == 0) {
							return string.Compare((x as MethodInfo).GetParameters().Length.ToString(), (y as MethodInfo).GetParameters().Length.ToString());
						}
						return i;
					}
					return string.Compare(x.Name, y.Name);
		...
		protected bool IsMatchSearch(object obj, string search) {
			...
			switch(browserState.searchKind) {
				...
				case SearchKind.Equals:
					return name.ToLower().Equals(search.ToLower());
				case SearchKind.Startwith:
					return name.ToLower().StartsWith(search.ToLower());
				case SearchKind.Endswith:
					return name.ToLower().EndsWith(search.ToLower());
			}
			return false;
		}
