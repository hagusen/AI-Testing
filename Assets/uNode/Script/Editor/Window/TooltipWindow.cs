			...
			if(contents != null) {
				...
				for(int i = 0; i < contents.Count; i++) {
					if(i == 0 && contents[i].image != null) {
						if(contents.Count > 1) {
							EditorGUI.LabelField(uNodeGUIUtility.GetRect(2), contents[i], uNodeGUIUtility.Styles.RichLabel);
						} else {
							EditorGUI.LabelField(uNodeGUIUtility.GetRect(2), new GUIContent(contents[i].image), uNodeGUIUtility.Styles.CenterRichLabel);
							EditorGUILayout.LabelField(contents[i].text, uNodeGUIUtility.Styles.RichLabel);
						}
						continue;
					}
					if(contents[i].image != null) {
						EditorGUI.LabelField(uNodeGUIUtility.GetRect(), contents[i], uNodeGUIUtility.Styles.RichLabel2);
					} else {
						EditorGUILayout.LabelField(contents[i], uNodeGUIUtility.Styles.RichLabel);
					}
				}
				if(isOver)
