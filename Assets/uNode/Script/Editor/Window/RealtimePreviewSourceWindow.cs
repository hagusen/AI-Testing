...
namespace MaxyGames.uNode.Editors {
	public class RealtimePreviewSourceWindow : PreviewSourceWindow {
		...
		[MenuItem("Tools/uNode/CSharp Preview")]
		...
		void Generate(object obj) {
			try {
				...
				var script = CodeGenerator.Generate(new CodeGenerator.GeneratorSetting(objs[0] as uNodeData, objs[1] as ICollection<uNodeRoot>) {
					debugScript = false,
					isAsync = true,
					isPreview = true,
					...
					fullTypeName = true,
					maxQueue = 15,
					generateTwice = generatorData.generateNodeTwice,
					enableOptimization = generatorData.enableOptimization,
					forceGenerateAllNode = generatorData.forceGenerateAllNode,
					fullComment = generatorData.fullComment,
					resolveUnityObject = generatorData.resolveUnityObject,
				});
				lock(lockThread) {
					exception = null;
					//this.script = script;
