...
namespace MaxyGames.uNode.Editors {
	public class GraphCreatorWindow : EditorWindow {
		...
		void OnGUI() {
			if (graphCreator == null) {
				graphCreator = FindGraphCreators().FirstOrDefault(item => item.displayName == "Class Component");
				if(graphCreator == null) {
			...
			if (uNodeEditorGUI.Button(new GUIContent(graphCreator.displayName), EditorStyles.popup)) {
	...
	public abstract class GraphCreator {
		...
		public virtual string displayName => menuName.Split('/').LastOrDefault();
