...
namespace MaxyGames.uNode.Editors {
	...
	public class GraphEditorData {
		...
		public bool supportCoroutine {
			get {
				if(selectedRoot == null && graph is IStateGraph state && state.canCreateGraph) {
					return true;
			...
			get {
				...
				if(_debugObject != null) {
					...
					if(Application.isPlaying && graph != null) {
						...
						if(uNodeUtility.debugData.ContainsKey(uNodeUtility.GetObjectID(obj))) {
							Dictionary<object, uNodeUtility.DebugData> debugMap = null;
							debugMap = uNodeUtility.debugData[uNodeUtility.GetObjectID(obj)];
							if(debugMap.Count > 0) {
		...
		public bool debugAnyScript, debugSelf = true;
		#endregion
