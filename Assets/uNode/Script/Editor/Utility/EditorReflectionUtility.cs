...
namespace MaxyGames.uNode.Editors {
	public static class EditorReflectionUtility {
		...
		public static void BuildRuntimeTypes() {
			var graphs = uNodeEditorUtility.FindComponentInPrefabs<IClassIdentifier>();
			var types = new List<RuntimeType>();
			foreach(var g in graphs) {
				if(!(g is uNodeRoot)) continue;
				types.Add(ReflectionUtils.GetRuntimeType(g as uNodeRoot));
			}
			var ifaces = uNodeEditorUtility.FindAssetsByType<uNodeInterface>();
		...
		public static Assembly[] GetAssemblies() {
			if(assemblies == null) {
				...
				for(int i = 0; i < ass.Count; i++) {
					try {
						if(string.IsNullOrEmpty(ass[i].Location) || ass[i].IsDynamic) {
							ass.RemoveAt(i);
							i--;
						}
					}
		...
		public static MemberInfo[] GetSortedMembers(Type type, BindingFlags flags) {
			...
			if(!dic.TryGetValue(flags, out var map)) {
				...
				Array.Sort(map, (x, y) => string.Compare(x.Name, y.Name));
				dic[flags] = map;
			}
			return map;
		}
		...
		public static void RenderVariable(Rect position, MemberData variable, GUIContent label, UnityEngine.Object unityObject, FilterAttribute filter = null, Action<MemberData> onChange = null) {
			...
			if(filter.UnityReference &&
				...
				DrawInstanceValue(rect, GUIContent.none, variable, unityObject, onChange);
				rect.x += rect.width;
		...
		public static void DrawInstanceValue(Rect position, GUIContent label, MemberData variable, UnityEngine.Object unityObject = null, Action<MemberData> onChange = null) {
			...
			if(t != null && !t.IsCastableTo(typeof(UnityEngine.Object)) || target is MemberData) {
				if(target == null && ReflectionUtils.CanCreateInstance(t)) {
					target = ReflectionUtils.CreateInstance(t);
		...
		public static float DrawVariableValues(GUIContent label, MemberData variable, UnityEngine.Object unityObject = null, Action<MemberData> onChange = null) {
			DrawVariableValues(uNodeGUIUtility.GetRectCustomHeight(17), label, variable, null, null, unityObject, onChange);
			return 17;
		}

		public static float DrawVariableValues(GUIContent label, MemberData variable, Type type, FilterAttribute filter, UnityEngine.Object unityObject = null, Action<MemberData> onChange = null) {
			DrawVariableValues(uNodeGUIUtility.GetRectCustomHeight(17), label, variable, type, filter, unityObject, onChange);
			return 17;
		}

		public static void DrawVariableValues(Rect position, GUIContent label, MemberData variable, UnityEngine.Object unityObject = null, Action<MemberData> onChange = null) {
			DrawVariableValues(position, label, variable, null, null, unityObject, onChange);
		}

		private static Dictionary<MemberData, object> _memberValueMap = new Dictionary<MemberData, object>();

		public static void DrawVariableValues(Rect position, GUIContent label, MemberData variable, Type type, FilterAttribute filter,
			...
			if(t != null) {
				...
				if(flag)
					position.width -= 16;
				if(type != null && variable.targetType != MemberData.TargetType.Values) {
					...
					if(filter.ValidTargetType == MemberData.TargetType.Values && !filter.InvalidTargetType.HasFlags(MemberData.TargetType.Values) && filter.IsValueTypes()) {
						variable.targetType = MemberData.TargetType.Values;
						variable.targetTypeName = t.FullName;
						if(onChange != null) {
							onChange(variable);
						}
						uNodeGUIUtility.GUIChanged(unityObject);
					...
					if(ReflectionUtils.CanCreateInstance(t) || t.IsCastableTo(typeof(UnityEngine.Object))) {
						if(string.IsNullOrEmpty(variable.targetTypeName)) {
							variable.targetTypeName = t.FullName;
						}
						if(!_memberValueMap.TryGetValue(variable, out obj)) {
							obj = variable.Get();
							_memberValueMap[variable] = obj;
						}
						if(obj != null && t.IsCastableTo(typeof(UnityEngine.Object)) && !obj.GetType().IsCastableTo(typeof(UnityEngine.Object))) {
							obj = null;
							variable.CopyFrom(MemberData.CreateFromValue(obj));
							if(onChange != null) {
								onChange(variable);
							}
							if(_memberValueMap.ContainsKey(variable)) {
								_memberValueMap.Remove(variable);
							}
							uNodeGUIUtility.GUIChanged(unityObject);
						}
				...
				if(flag) {
					...
					position.width = 16;
					bool check = EditorGUI.Toggle(position, variable.targetType != MemberData.TargetType.Values, EditorStyles.radioButton);
					if(check && variable.targetType == MemberData.TargetType.Values) {
						//string tName = variable.targetTypeName;
						if(unityObject)
							uNodeEditorUtility.RegisterUndo(unityObject, "");
						//variable.Reset();
						//variable.targetTypeName = tName;
						variable.targetType = t.IsValueType ? MemberData.TargetType.None : MemberData.TargetType.Null;
						variable.ResetCache();
						if(onChange != null) {
							onChange(variable);
						}
						uNodeGUIUtility.GUIChanged(unityObject);
					} else if(!check && variable.targetType != MemberData.TargetType.Values) {
						if(unityObject)
							uNodeEditorUtility.RegisterUndo(unityObject, "");
						variable.targetType = MemberData.TargetType.Values;
						variable.targetTypeName = t.FullName;
		...
		public static void DrawMemberValues(GUIContent label, MemberData member, Type type, FilterAttribute filter,
			...
			if(t != null) {
				...
				object obj;
				if(ReflectionUtils.CanCreateInstance(t) || t.IsCastableTo(typeof(UnityEngine.Object))) {
					if(string.IsNullOrEmpty(member.targetTypeName)) {
						member.targetTypeName = t.FullName;
					}
		...
		public static void ShowGUI(Rect position, MemberData variable, FilterAttribute filter = null, UnityEngine.Object unityObject = null, Action<MemberData> onChange = null) {
			...
			if(!filter.OnlyGetType &&
				...
				DrawVariableValues(position, GUIContent.none, variable, t, filter, unityObject, onChange);
			} else {
				DrawVariableReference(position, variable, filter, unityObject, onChange);
			}
		}
		...
		public static bool ValidateMember(MemberInfo member, FilterAttribute filter) {
			if(!uNodePreference.GetPreference().showObsoleteItem && member.GetCustomAttributes(typeof(ObsoleteAttribute), false).Length > 0) {
				return false;
			}
			if(filter != null) {
				...
				if(filter.NonPublic && !filter.Private && !IsPublicMember(member) && !IsProtectedMember(member)) {
					return false;
				}
			}
			switch(member.MemberType) {
				case MemberTypes.Method: {
					...
					if(member.Name.StartsWith("get_") || member.Name.StartsWith("set_") || member.Name.StartsWith("op_") || member.Name.StartsWith("add_") || member.Name.StartsWith("remove_")) {
						if(!member.Name.StartsWith("get_Item") && !member.Name.StartsWith("set_Item")) {
							return false;
						}
					}
				}
		...
		public static bool ValidateItem(ReflectionItem item, FilterAttribute filter) {
			if(!uNodePreference.GetPreference().showObsoleteItem && item.memberInfo.GetCustomAttributes(typeof(ObsoleteAttribute), false).Length > 0) {
				return false;
			}
			if(filter != null) {
				bool valid = filter.IsValidType(item.memberType);
				if(!valid) {
					return false;
				}
				bool flag = filter.ValidTargetType != MemberData.TargetType.None;
				...
				if(filter.NonPublic && !filter.Private && !IsPublicMember(item.memberInfo) && !IsProtectedMember(item.memberInfo)) {
					return false;
				}
			}
			switch(item.memberInfo.MemberType) {
				case MemberTypes.Method: {
					MethodInfo info = item.memberInfo as MethodInfo;
					int minMethodParam = 0;
					int maxMethodParam = 0;
					if(filter != null) {
						minMethodParam = filter.MinMethodParam;
					...
					if((item.memberInfo.Name.StartsWith("get_") || item.memberInfo.Name.StartsWith("set_")) &&
						info.GetParameters().Length == 0 || item.memberInfo.Name.StartsWith("op_")) {
						if(!item.memberInfo.Name.StartsWith("get_Item") && !item.memberInfo.Name.StartsWith("set_Item")) {
							return false;
						}
					}
				}
				break;
				case MemberTypes.Property: {
					PropertyInfo info = item.memberInfo as PropertyInfo;
					if(info != null) {
						ParameterInfo[] pInfo = info.GetIndexParameters();
						if(pInfo != null && pInfo.Length > 0) {
							return false;
			...
			if(!filter.Boxing) {
				return !item.memberInfo.DeclaringType.IsValueType;
			}
			return filter.ValidMemberType.HasFlags(item.memberInfo.MemberType);
		}
			...
			switch(member.MemberType) {
				case MemberTypes.Method: {
					...
					if((member.Name.StartsWith("get_") || member.Name.StartsWith("set_")) &&
						info.GetParameters().Length == 0 || member.Name.StartsWith("op_")) {
						if(!member.Name.StartsWith("get_Item") && !member.Name.StartsWith("set_Item")) {
							return false;
						}
					}
				...
				foreach(var type in types) {
					if(type.IsGenericTypeDefinition) {
						if(type.GetGenericArguments().Length == genericArgument.Length) {
							try {
								typesList.Add(type.MakeGenericType(genericArgument));
							}
							catch { }
						}
					}
		...
		public static List<ReflectionItem> GetChildItems(Type type, FilterAttribute filter, Func<ReflectionItem, bool> validation = null, int deep = 0, int maxDeep = 1) {
			if(deep >= maxDeep)
				return null;
			var items = AddReflectionItems(filter, type);
			if(deep + 1 >= maxDeep)
				return items;
			foreach(var item in items) {
				if(item.isStatic && item.memberInfo != null && item.memberInfo.MemberType == MemberTypes.NestedType) {
					GetChildItems(item.memberType, filter, validation, deep + 1, maxDeep);
				}
			}
			return items;
		}

		public static List<ReflectionItem> AddReflectionItems(FilterAttribute filter, Type type,
			Type startType = null, Func<ReflectionItem, bool> validation = null,
			Func<MemberInfo, bool> memberValidation = null) {
			BindingFlags flags = filter.validBindingFlags;
			return GetReflectionItems(type, flags, filter, startType, validation, memberValidation);
		}
		...
		public static List<ReflectionItem> AddGeneralReflectionItems(Type memberType, FilterAttribute filter, Func<ReflectionItem, bool> validation = null) {
			if(memberType != null) {
				return GetReflectionItems(memberType, filter.validBindingFlags, filter, null, validation);
			}
			return null;
		}
		...
		public static List<ReflectionItem> AddGeneralReflectionItems(MemberData member, FilterAttribute filter, Func<ReflectionItem, bool> validation = null) {
			if(member.instance != null) {
				return GetReflectionItems(member.instance.GetType(), filter.validBindingFlags, filter, null, validation);
			}
			return null;
		}
		...
		public static ReflectionItem GetReflectionItems(MemberInfo info,
			...
			return GetReflectionItems(info, startType, filter.validBindingFlags, filter, validation);
		}
		...
		public static ReflectionItem GetReflectionItems(MemberInfo info,
			...
			if(canSelect && filter.ValidMemberType.HasFlags(info.MemberType)) {
				bool flag = filter.SetMember ? ReflectionUtils.CanSetMemberValue(info) : ReflectionUtils.CanGetMember(info);
				if(flag) {
					...
					bool canGet = ReflectionUtils.CanGetMember(info);
					if(canGet) {
						ReflectionItem item = new ReflectionItem() {
							hasNextItems = true,
							memberInfo = info,
				...
				bool flag = ReflectionUtils.CanGetMember(info);
				if(flag) {
					ReflectionItem item = new ReflectionItem() {
						hasNextItems = true,
						memberInfo = info,
		...
		public static List<ReflectionItem> GetReflectionItems(Type type,
			...
			if(filter.Static && !filter.SetMember && filter.ValidMemberType.HasFlags(MemberTypes.Constructor) && !type.IsCastableTo(typeof(Delegate))) {
				...
				for(int i = ctor.Length - 1; i >= 0; i--) {
					ReflectionItem item = GetReflectionItems(ctor[i], startType, bindingFlags, filter, validation, memberValidation);
					if(item != null) {
						Items.Add(item);
					}
				}
			...
			for(int i = 0; i < members.Length; i++) {
				ReflectionItem item = GetReflectionItems(members[i], startType, bindingFlags, filter, validation, memberValidation);
		...
		public static void SortItem(List<ReflectionItem> items) {
			items.Sort((x, y) => string.Compare(
					x.memberInfo == null || x.memberInfo.MemberType != MemberTypes.Constructor ? x.name : "@",
					y.memberInfo == null || y.memberInfo.MemberType != MemberTypes.Constructor ? y.name : "@"));
		}
		...
		public static List<ReflectionItem> GetReflectionItems(Type type, IEnumerable<MemberInfo> members, BindingFlags bindingFlags,
			...
			foreach(var member in members) {
				if(type == null) {
					startType = member.DeclaringType;
				}
				ReflectionItem item = GetReflectionItems(member, startType, bindingFlags, filter, validation, memberValidation);
				if(item != null && (item.memberInfo == null || item.memberInfo.MemberType != MemberTypes.Constructor)) {
					Items.Add(item);
				}
			}
		...
		public class ReflectionItem : IComparable<ReflectionItem> {
			...
			public bool displayBaseMember;

			public List<TypeData> genericArguments;
			public List<UnityEngine.Object> genericObjects;
						...
						if(memberInfo != null) {
							switch(memberInfo.MemberType) {
								...
								case MemberTypes.Method:
									...
									if(displayBaseMember) {
										_displayName = memberInfo.DeclaringType.PrettyName() + "." + _displayName;
									}
									break;
								case MemberTypes.NestedType:
								case MemberTypes.TypeInfo:
			...
			public int CompareTo(ReflectionItem other) {
				return string.Compare(
					memberInfo == null || memberInfo.MemberType != MemberTypes.Constructor ? name : "@",
					other.memberInfo == null || other.memberInfo.MemberType != MemberTypes.Constructor ? other.name : "@");
			}

			public ReflectionItem() { }
			public ReflectionItem(ReflectionItem other) {
				this.memberInfo = other.memberInfo;
				this.isStatic = other.isStatic;
				this.memberType = other.memberType;
				this.hasNextItems = other.hasNextItems;
				this.canSelectItems = other.canSelectItems;
				this.genericArguments = other.genericArguments;
				this.genericObjects = other.genericObjects;
