	...
	public static class uNodeEditorUtility {
		...
		public static void RegisterUndoSetTransformParent(Transform transform, Transform newParent, string name = "") {
			if(IsPrefabInstance(transform)) {
				UnlockPrefabInstance(EditorBinding.getPrefabParent(transform) as GameObject);
		...
		public static List<CustomGraphAttribute> FindCustomGraph() {
			if(_customGraphs == null) {
				...
				_customGraphs.Sort((x, y) => string.Compare(x.name, y.name));
			}
		...
		public static List<T> FindAllAssetsByType<T>() where T : UnityEngine.Object {
			...
			for(int i = 0; i < guids.Length; i++) {
				...
				if(asset != null) {
                    assets.AddRange(asset.Select(a => a as T).Where(a => a != null));
                }
			}
			return assets;
		}
		...
		/// <param name="prefab"></param>
		public static GameObject SavePrefabAsset(GameObject gameObject, UnityEngine.Object prefab) {
...
#else
			return PrefabUtility.ReplacePrefab(gameObject, prefab);
#endif
		}
