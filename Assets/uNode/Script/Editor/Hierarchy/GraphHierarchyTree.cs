...
namespace MaxyGames.uNode.Editors {
	#region TreeViews
	internal class HierarchyGraphTree : TreeViewItem {
	...
	public class GraphHierarchyTree : TreeView {
		...
		protected override void RowGUI(RowGUIArgs args) {
			...
			if(evt.type == EventType.Repaint) {
				...
				if(args.item is HierarchyNodeTree || args.item is HierarchyRefNodeTree) {
					...
					if(node != null && Application.isPlaying && uNodeUtility.useDebug) {
					...
					if(node != null && flow != null && Application.isPlaying && uNodeUtility.useDebug) {
						var debugData = GetDebugInfo();
						if(debugData != null) {
							var oldColor = GUI.color;
							float times = -1;
				...
				if(args.item is HierarchySummaryTree) {
					uNodeGUIUtility.Styles.itemNormal.Draw(labelRect, new GUIContent(uNodeUtility.WrapTextWithColor("//" + args.label, uNodeUtility.GetRichTextSetting().summaryColor), args.item.icon), false, false, false, false);
				} else {
					...
					uNodeGUIUtility.Styles.itemNormal.Draw(labelRect, new GUIContent(args.label, args.item.icon), false, false, false, false);
				}
				#endregion
			}
			//base.RowGUI(args);
		...
		private uNodeUtility.DebugData GetDebugInfo() {
			if(editorData.graph != graph)
				return null;
			object debugObject = editorData.debugTarget;
			if(Application.isPlaying && debugObject != null) {
				if(debugObject is uNodeUtility.DebugData) {
					return debugObject as uNodeUtility.DebugData;
				} else if(uNodeUtility.debugData.TryGetValue(uNodeUtility.GetObjectID(editorData.graph), out var debugMap) &&
