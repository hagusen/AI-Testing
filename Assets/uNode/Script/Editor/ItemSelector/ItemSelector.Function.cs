...
namespace MaxyGames.uNode.Editors {
	public partial class ItemSelector {
		...
		void Setup() {
			...
			if(filter == null) {
				filter = new FilterAttribute() { UnityReference = false, OnlyGetType = onlyGetType };
			}
			if(onlyGetType)
				filter = new FilterAttribute(filter) { OnlyGetType = onlyGetType };
			...
			for(int i = 0; i < assemblies.Length; i++) {
				...
				foreach(var type in EditorReflectionUtility.GetAssemblyTypes(assembly)) {
					...
					if(string.IsNullOrEmpty(ns)) {
						ns = "global";
						if(!includeGlobal && !namespaces.Contains(ns)) {
							continue;
						}
					} else {
