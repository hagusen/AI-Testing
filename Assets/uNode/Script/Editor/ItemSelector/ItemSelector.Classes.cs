	...
	public partial class ItemSelector {
		...
		internal class SelectorCustomTreeView : TreeViewItem {
			...
			public SelectorCustomTreeView(CustomItem item, int id, int depth) : base(id, depth, item.name) {
				this.item = item;
				icon = item.icon as Texture2D ?? uNodeEditorUtility.GetTypeIcon(typeof(TypeIcons.ExtensionIcon)) as Texture2D;
			}
		...
		class Data : IDisposable {
			...
			public SearchKind searchKind = SearchKind.Contains;
		...
		public class CustomItem {
			...
			public CustomItem() { }
			public CustomItem(string name, EditorReflectionUtility.ReflectionItem item) {
				this.name = name;
				this.item = item;
			}

			public CustomItem(string name, Action onClick, string category = "Data") {
				this.name = name;
				this.onClick = delegate (object obj) {
					onClick();
				};
				this.category = category;
			}

			public CustomItem(string name, Action<object> onClick, object userObject, string category = "Data") {
				this.name = name;
			...
			public CustomItem(GraphItem item, string category = "Data") {
				name = "#ESItem";
				this.category = category;
				userObject = item;
			}
		...
		public static class TreeFunction {
			public static List<TypeTreeView> GetGeneralTrees() {
				...
				types.Sort((x, y) => string.Compare(x.Name, y.Name));
			...
			public static MemberTreeView CreateItemFromMember(MemberInfo member, FilterAttribute filter) {
				...
				if(canSelect && filter.ValidMemberType.HasFlags(member.MemberType)) {
					bool flag = filter.SetMember ? ReflectionUtils.CanSetMemberValue(member) : ReflectionUtils.CanGetMember(member);
					if(flag) {
						...
						bool canGet = ReflectionUtils.CanGetMember(member);
						if(canGet) {
							return new MemberTreeView(member, member.GetHashCode(), -1) {
								selectValidation = () => EditorReflectionUtility.ValidateMember(member, filter),
								nextValidation = () => EditorReflectionUtility.ValidateNextMember(member, filter),
					...
					bool flag = ReflectionUtils.CanGetMember(member);
					if(flag) {
						return new MemberTreeView(member, member.GetHashCode(), -1) {
							selectValidation = () => EditorReflectionUtility.ValidateMember(member, filter),
							nextValidation = () => EditorReflectionUtility.ValidateNextMember(member, filter),
			...
			public static List<TreeViewItem> CreateGraphItem(Object targetObject, MemberData reflectionValue, FilterAttribute filter) {
				...
				if(targetObject) {
					...
					if(graph != null) {
						...
						graphItem.Sort((x, y) => string.Compare(x.Name, y.Name));
				...
				if(targetUNR != null && targetUNR != graph) {
					...
					itemData.Sort((x, y) => string.Compare(x.Name, y.Name));
			...
			public static List<TreeViewItem> CreateCustomItem(List<CustomItem> customItems) {
				var result = new List<TreeViewItem>();
				if(customItems != null && customItems.Count > 0) {
					var CustomItems = new Dictionary<string, List<CustomItem>>();
					foreach(var item in customItems) {
						string[] path = item.category.Split('/');
						string _p = null;
						foreach(string p in path) {
							...
							_p += p;
							if(CustomItems.ContainsKey(_p)) {
								CustomItems[_p].Add(item);
							} else {
								CustomItems[_p] = new List<CustomItem>() { item };
							}
						}
					}
					if(CustomItems != null && CustomItems.Count > 0) {
						foreach(var pair in CustomItems) {
							var categTree = new SelectorCategoryTreeView(pair.Key, "", uNodeEditorUtility.GetUIDFromString("[CITEM]" + pair.Key), -1);
							pair.Value.ForEach(item => categTree.AddChild(new SelectorCustomTreeView(item, item.GetHashCode(), -1)));
							result.Add(categTree);
						}
					}
				}
