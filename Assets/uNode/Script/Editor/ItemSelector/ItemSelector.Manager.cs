...
namespace MaxyGames.uNode.Editors {
	public partial class ItemSelector {
		public sealed class Manager : TreeView, IDisposable {
			...
			private FilterAttribute filter => editorData?.filter;
			...
			private List<GUIContent> LoadDoc(MemberInfo member) {
				...
				if(preferenceData.showDocumentation && XmlDoc.hasLoadDoc) {
					if(member != null) {
					...
					if(args.rowRect.Contains(evt.mousePosition)) {
						if(hoveredTree != args.item) {
							...
							if(args.item is TypeTreeView) {
								...
								contents.Add(new GUIContent(item.displayName, icon));
								contents.Add(new GUIContent("Target	: " + item.member.MemberType));
								...
								if(preferenceData.showDocumentation && XmlDoc.hasLoadDoc) {
									if(item.member is ISummary summary) {
										if(!string.IsNullOrEmpty(summary.GetSummary())) {
											contents.Add(new GUIContent("<b>Documentation</b> ▼ " + summary.GetSummary().AddLineInFirst()));
										}
										if(item.member is RuntimeMethod) {
											var parameters = (item.member as RuntimeMethod).GetParameters();
											for(int x = 0; x < parameters.Length; x++) {
												var PType = parameters[x].ParameterType;
												contents.Add(new GUIContent("<b>" + parameters[x].Name + " : " + PType.PrettyName() + "</b>",
														uNodeEditorUtility.GetTypeIcon(PType)));
				...
				if(args.item is SelectorCategoryTreeView) {
					...
					if(evt.type == EventType.Repaint) {
						if(canSelect) {
							...
							pos.width = 15;
							uNodeGUIUtility.Styles.itemSelect.Draw(pos, GUIContent.none, false, false, false, false);
						}
						if(canNext) {
							Rect pos = labelRect;
							pos.x += labelRect.width - 15;
							pos.width = 15;
							uNodeGUIUtility.Styles.itemNext.Draw(pos, GUIContent.none, false, false, false, false);
						}
						if(canSelect) {
							labelRect.width -= 25;
						} else if(canNext) {
						...
						if(args.rowRect.Contains(evt.mousePosition)) {
							if(args.item is MemberTreeView) {
								...
								GUI.DrawTexture(pos, isFavorited ? uNodeGUIUtility.Styles.favoriteIconOn : uNodeGUIUtility.Styles.favoriteIconOff);
								labelRect.width -= pos.width;
						...
						if(IsStaticTree(args.item)) {
							uNodeGUIUtility.Styles.itemStatic.Draw(labelRect, new GUIContent(args.label, icon), false, false, false, false);
						} else {
							uNodeGUIUtility.Styles.itemNormal.Draw(labelRect, new GUIContent(args.label, icon), false, false, false, false);
						}
				...
				if(tree is TypeTreeView) {
					...
					if(item.item != null) {
						if(item.item.onClick != null) {
							item.item.onClick(item.item.userObject);
						} else if(item.item.item != null) {
							var member = item.item.item.memberInfo;
							if(ResolveGenericItem(member, false, (mInfo) => {
								if(member is Type) {
									deepTrees.Add(new TypeTreeView(mInfo as Type, tree.id, tree.depth));
								} else {
									deepTrees.Add(new MemberTreeView(mInfo, tree.id, tree.depth) {
										instance = item.item.item.instance
									});
								}
								SelectDeepTrees();
								GUIUtility.ExitGUI();
			...
			public void SelectDeepTrees() {
				...
				if(lastItem is TypeTreeView) {
					...
					if(filter.Types?.Count == 1 && filter.Types[0] == typeof(Type) && !(type is RuntimeType)) {
						...
						if(ReflectionUtils.CanCreateInstance(type)) {
							val = MemberData.CreateFromValue(ReflectionUtils.CreateInstance(type), type);
						} else {
				...
				for(int i = 0; i < members.Count; i++) {
					...
					mData.targetTypeName = member.targetTypeName;
			...
			public bool CanSelectTree(TreeViewItem tree) {
				if(tree is TypeTreeView) {
					...
					if(item.item != null) {
						if(item.item.onClick != null)
							return true;
						if(item.item.item != null && item.item.item.canSelectItems) {
							return true;
						}
					}
					if(item.graphItem != null && item.graphItem.type != null) {
						return IsValidTypeToSelect(item.graphItem.type);
					}
			...
			void SelectGraphItem(GraphItem item) {
					...
					if(genericParamArgs.Length > 0) {
						...
						for(int i = 0; i < genericParamArgs.Length; i++) {
							if(item.genericParameterTypes[i].targetType == MemberData.TargetType.uNodeGenericParameter) {
								...
								param[i] = new TypeData() { name = item.genericParameterTypes[i].startTypeName };
							}
						}
				...
				if(item.type != null) {
					member.targetTypeName = item.type.FullName;
				}
							...
							TypeSelectorWindow.ShowAsNew(Rect.zero, typeItems[0].filter, delegate (MemberData[] types) {
								...
								if(!hasGenericParameterTarget) {
									if(method != null && method.IsGenericMethodDefinition) {
										...
										mType = mType.MakeGenericType(types.Select(i => i.Get<Type>()).ToArray());
										member = mType;
									} else {
			...
			private Texture GetIcon(TreeViewItem tree) {
				if(tree is MemberTreeView) {
					...
					var item = tree as SelectorCustomTreeView;
					if(item.item != null && item.item.item != null) {
						return uNodeEditorUtility.GetTypeIcon(item.item.item.memberType);
					}
				}
				return tree.icon;
			}

			private Texture2D GetSecondIcon(TreeViewItem tree) {
				if(tree is TypeTreeView) {
					return null;
				} else if(tree is MemberTreeView) {
					var item = tree as MemberTreeView;
					return uNodeEditorUtility.GetTypeIcon(ReflectionUtils.GetMemberType(item.member)) as Texture2D;
				} else if(tree is SelectorCustomTreeView) {
					var item = tree as SelectorCustomTreeView;
			...
			class TreeSearch {
				...
				public List<TreeViewItem> Search(List<TreeViewItem> treeViews, string searchString, SearchKind searchKind, SearchFilter searchFilter) {
					...
					try {
						...
						for(int i = 0; i < treeViews.Count; i++) {
							if(IsValidSearch(treeViews[i], new SearchParam(searchString, searchKind, searchFilter), progresses)) {
				...
				bool IsValidSearch(TreeViewItem tree, SearchParam searchParam, List<SearchProgress> progresses, int depth = 1) {
					if(tree is TypeTreeView && deepSearch) {
						var item = tree as TypeTreeView;
						if(!item.type.IsEnum && searchParam.searchString.Length >= 3) {
							if(searchParam.splittedStrings.Length < 2 ||  !string.IsNullOrEmpty(searchParam.splittedStrings[1])) {
								if(!searchParam.isUsingDot || IsMatchSearch(tree.displayName, searchParam.splittedStrings[0], searchParam.searchKind)) {
									item.Search((member) => {
										return IsMatchSearch(member, tree.displayName, searchParam.splittedStrings, searchParam.searchKind, searchParam.searchFilter);
									});
									return tree.hasChildren && tree.children.Count > 0 || IsMatchSearch(tree, searchParam.splittedStrings, searchParam.searchKind, searchParam.searchFilter);
								}
							}
						}
					...
					if(tree.hasChildren) {
						...
						for(int i = 0; i < childs.Count; i++) {
							...
							if(!IsValidSearch(childs[i], searchParam, progresses, depth + 1)) {
								childs.RemoveAt(i);
								i--;
							}
						}
					}
					return tree is SelectorSearchTreeView || tree.hasChildren && tree.children.Count > 0 || IsMatchSearch(tree, searchParam.splittedStrings, searchParam.searchKind, searchParam.searchFilter);
				}
				...
				bool IsMatchSearch(MemberInfo member, string typeName, string[] splittedStrings, SearchKind searchKind, SearchFilter searchFilter) {
					bool flag = true;
					switch(searchFilter) {
				...
				bool IsMatchSearch(TreeViewItem tree, string[] splittedStrings, SearchKind searchKind, SearchFilter searchFilter) {
				...
				bool IsMatchSearch(string str, string searchString, SearchKind searchKind) {
					if(string.IsNullOrEmpty(searchString))
						return true;
					switch(searchKind) {
						case SearchKind.Contains:
							return str.IndexOf(searchString, StringComparison.CurrentCultureIgnoreCase) >= 0;
						case SearchKind.Endwith:
							return str.EndsWith(searchString, StringComparison.CurrentCultureIgnoreCase);
						case SearchKind.Startwith:
							return str.StartsWith(searchString, StringComparison.CurrentCultureIgnoreCase);
						case SearchKind.Equal:
							return str.Equals(searchString, StringComparison.CurrentCultureIgnoreCase);
					}
					return false;
				}
					...
					if(isDeep) {
						...
						if(treeView is MemberTreeView) {
							...
							var trees = new List<TreeViewItem>();
							{
								...
								namespaces.RemoveAll(n => n == null || !n.StartsWith(ns) || n.Length <= ns.Length);
								namespaces.Sort();
