...
namespace MaxyGames.Events {
	...
	public class EventDataDrawer : PropertyDrawer {
		...
		public override void OnGUI(Rect position, SerializedProperty property, UnityEngine.GUIContent label) {
			...
			eventData = PropertyDrawerUtility.GetActualObjectForSerializedProperty<EventData>(property);
			{
				...
				if(Event.current.type == EventType.Repaint) {
					uNodeGUIUtility.Styles.headerStyle.Draw(rect, GUIContent.none, controlID);
				}
				...
				isExpanded = EditorGUI.Foldout(rect, isExpanded, new GUIContent(property.displayName, toolTip), uNodeGUIUtility.Styles.FoldoutBold);
			...
			if(property.isExpanded && eventData != null) {
				...
				if(Event.current.type == EventType.Repaint) {//Background
					...
					uNodeGUIUtility.Styles.backgroundStyle.Draw(bgRect, false, false, false, false);
				}
				if(eventData.blocks != null && eventData.blocks.Count > 0 && !isActionEvent) {
					for(var i = 0; i < eventData.blocks.Count; i++) {
						...
						if(eventData.blocks[i].block is MissingEvent) {
							...
							if(eventPos.Contains(Event.current.mousePosition) && (Event.current.button == 0 && Event.current.clickCount == 2 || Event.current.button == 1 && Event.current.type == EventType.MouseUp)) {
								...
								ItemSelector.ShowWindow(null, filter, delegate (MemberData value) {
									System.Type RType = TypeSerializer.Deserialize(value.targetTypeName, false);
							...
							if(GUI.Button(eventPos, new GUIContent("<size=11>Missing Block</size>", eventData.blocks[i].TypeName +
								"\n\nRight Click to select correct type"), uNodeGUIUtility.Styles.ButtonRichText)) {
						...
						if(eventData.blocks[i].eventType == EventActionData.EventType.Event) {
							...
							if(i == 0) {
								...
								if(GUI.Button(eventPos, new GUIContent("if(<size=11>" + eventData.blocks[i].displayName + "</size>" + s, uNodeEditorUtility.RemoveHTMLTag(eventData.blocks[i].toolTip)), uNodeGUIUtility.Styles.ButtonRichText)) {
									eventData.editIndex = i + 1;
								}
							} else if(i == eventData.blocks.Count - 1) {
								if(GUI.Button(eventPos, new GUIContent("<size=11>" + eventData.blocks[i].displayName + ")</size>", uNodeEditorUtility.RemoveHTMLTag(eventData.blocks[i].toolTip)), uNodeGUIUtility.Styles.ButtonRichText)) {
									eventData.editIndex = i + 1;
								}
							} else {
								...
								if(GUI.Button(eventPos, new GUIContent("<size=11>" + eventData.blocks[i].displayName + "</size>" + s, uNodeEditorUtility.RemoveHTMLTag(eventData.blocks[i].toolTip)), uNodeGUIUtility.Styles.ButtonRichText)) {
									eventData.editIndex = i + 1;
								}
							}
							GUI.backgroundColor = Color.white;
							...
							GUIStyle style = uNodeGUIUtility.Styles.ButtonRichText;
					...
					for(var i = 0; i < eventData.blocks.Count; i++) {
						...
						if(eventData.blocks[i].eventType == EventActionData.EventType.Event) {
							...
							if(GUI.Button(eventPos, new GUIContent(eventData.blocks[i].displayName, uNodeEditorUtility.RemoveHTMLTag(eventData.blocks[i].toolTip)), uNodeGUIUtility.Styles.ButtonRichText)) {
								eventData.editIndex = i + 1;
							}
							GUI.backgroundColor = Color.white;
						} else {
		...
		public static List<BlockMenuAttribute> FindAllMenu() {
			...
			menuItems.Sort((x, y) => string.Compare(x.category + x.name, y.category + y.name));
