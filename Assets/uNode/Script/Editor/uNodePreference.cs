...
namespace MaxyGames.uNode.Editors {
	public class uNodePreference {
		...
		public class PreferenceData {
			...
			public PreferredEditor preferredEditor = PreferredEditor.Node;
...
-Compatibility: is forcing to generate c# script that's compatible with all graph even when other graph is not compiled into script.")]
				...
				[Tooltip("If true, will try to optimize the generated code to better in term of readable and performance but this might slow down the code gen especially for huge graph")]
				...
				[Tooltip("If true, will auto creating variable for UnityEngine.Object and its sub classes type.")]
				public bool resolveUnityObject = true;
				[Tooltip("by default node that aren't connected are not generated but when this true all node include non conected node will be forced to generate.")]
				public bool forceGenerateAllNode = false;
				[Tooltip("If true, will try to generate all node twice.\nNode: all generated node will not include in output script this just for fix some issue.")]
				public bool generateNodeTwice = true;
		...
		public static bool enableEditOnNode = true;
		...
		public static void PreferencesGUI() {
			Initialize();
			selectedMenu = GUILayout.Toolbar(selectedMenu, new string[] { "Editor", "Type", "Code Generation" });
			scrollPos = EditorGUILayout.BeginScrollView(scrollPos);
			EditorGUI.BeginChangeCheck();
			if(selectedMenu == 0) {//Editor
				...
				if(!string.IsNullOrEmpty(preferenceData.editorTheme) && editorTheme != null && AssetDatabase.Contains(editorTheme)) {
					...
					if(editorTheme.expanded) {
						Editor editor = Editor.CreateEditor(editorTheme);
				...
				preferenceData.preferredEditor = (PreferredEditor)EditorGUILayout.EnumPopup(new GUIContent("Preferred Editor"), preferenceData.preferredEditor);
				...
				preferenceData.inEditorDocumentation = EditorGUILayout.Toggle(new GUIContent("In Editor Documentation"), preferenceData.inEditorDocumentation);
				...
				if(advanced) {
					...
					preferenceData.hideUNodeInstance = EditorGUILayout.Toggle(new GUIContent("Hide uNode Instance"), preferenceData.hideUNodeInstance);
					preferenceData.hideChildObject = EditorGUILayout.Toggle(new GUIContent("Hide Child Object"), preferenceData.hideChildObject);
					preferenceData.preventEditingPrefab = EditorGUILayout.Toggle(new GUIContent("Prevent Edit Prefab", "Prevent prefab editing on play mode."), preferenceData.preventEditingPrefab);
				...
				preferenceData.showGrid = EditorGUILayout.Toggle(new GUIContent("Show Grid"), preferenceData.showGrid);

				EditorGUILayout.LabelField("Node Browser", EditorStyles.boldLabel);
				uNodeGUIUtility.ShowField(nameof(preferenceData.itemSelectorSeparator), preferenceData);
				...
				preferenceData.minDeepTypeSearch = EditorGUILayout.IntField(
					new GUIContent(
						"Min Text For Deep Search",
						"The minimal text for performing deep type search"),
					...
					(position) => {
						...
						foreach(var assembly in ass) {
							...
							if(!string.IsNullOrEmpty(assemblyName) && !preferenceData.includedAssemblies.Contains(assemblyName)) {
								items.Add(new ItemSelector.CustomItem(assemblyName, (obj) => {
