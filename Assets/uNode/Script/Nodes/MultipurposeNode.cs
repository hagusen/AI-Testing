	...
	public class MultipurposeNode : ValueNode {
		...
		public override bool IsFlowNode() {
			if(target.target.isTargeted) {
				bool flag = target.target.targetType == MemberData.TargetType.Method ||
					...
					target.target.targetType == MemberData.TargetType.Constructor;
				if(flag)
					return true;
		...
		public override string GenerateValueCode() {
			return CodeGenerator.ParseValue(target, null, true);
		}

		public override string GenerateCode() {
			return CodeGenerator.ParseValue(target).AddSemicolon() + CodeGenerator.GetFinishCode(this, true, false, false, onFinished).AddLineInFirst();
		}
