...
namespace MaxyGames {
	public static partial class CodeGenerator {
		...
		public class GData {
			...
			public HashSet<NodeComponent> connectedNode = new HashSet<NodeComponent>();
			public HashSet<NodeComponent> groupedNode = new HashSet<NodeComponent>();
			public HashSet<NodeComponent> ungroupedNode = new HashSet<NodeComponent>();
			public HashSet<Node> nonBreakNode = new HashSet<Node>();
			...
			public Dictionary<Node, int> ConnectedInNodeCount = new Dictionary<Node, int>();
			public Dictionary<Node, IList<NodeComponent>> EventConnectedInNode = new Dictionary<Node, IList<NodeComponent>>();
			public Dictionary<Node, HashSet<NodeComponent>> FlowConnectedTo = new Dictionary<Node, HashSet<NodeComponent>>();
			...
			public Dictionary<Block, string> eventActions = new Dictionary<Block, string>();
			public Dictionary<object, CoroutineData> coroutineEvent = new Dictionary<object, CoroutineData>();

			public Dictionary<MemberData, KeyValuePair<int, string>> debugMemberMap = new Dictionary<MemberData, KeyValuePair<int, string>>();
			public Dictionary<Transform, HashSet<NodeComponent>> nodesMap = new Dictionary<Transform, HashSet<NodeComponent>>();
			public Dictionary<NodeComponent, HashSet<NodeComponent>> flowNodeConnectionsMap = new Dictionary<NodeComponent, HashSet<NodeComponent>>();
			public Dictionary<NodeComponent, HashSet<NodeComponent>> nodeConnectionsMap = new Dictionary<NodeComponent, HashSet<NodeComponent>>();
			...
			public Dictionary<string, Dictionary<Type, Dictionary<string, string>>> customUIDMethods = new Dictionary<string, Dictionary<Type, Dictionary<string, string>>>();

			public Dictionary<string, string> memberMap = new Dictionary<string, string>();
			public Dictionary<GlobalVariable, VariableData> globalVariableMap = new Dictionary<GlobalVariable, VariableData>();
			public Dictionary<UnityEngine.Object, string> unityVariableMap = new Dictionary<UnityEngine.Object, string>();

			public Dictionary<object, Dictionary<FieldInfo, Dictionary<int, string>>> fieldVariableMap = new Dictionary<object, Dictionary<FieldInfo, Dictionary<int, string>>>();
			...
			public string GetMethodName(EventNode method) {
				if(method == null)
					throw new System.Exception("method can't null");
				if(methodName.ContainsKey(method)) {
					return methodName[method];
				}
				string name = generatorData.GenerateMethodName(method.Name);
			...
			public MData AddMethod(string methodName, string returnType, params MPData[] parametersType) {
				if(string.IsNullOrEmpty(methodName) || returnType == null)
					throw new System.Exception("Method name or return type can't null");
        ...
        public class GeneratorSetting {
			...
			public bool resolveUnityObject;
			public bool fullComment = false;
			public bool generateTwice = true;
			public bool enableOptimization = false;
			...
			/// <summary>
			/// if false only connected node will be generated.
			/// </summary>
			public bool forceGenerateAllNode;
			...
			public GeneratorSetting(GameObject gameObject, GeneratorSetting settings) {
				...
				if(settings != null) {
					...
					resolveUnityObject = settings.resolveUnityObject;
					fullComment = settings.fullComment;
					generateTwice = settings.generateTwice;
				...
				if(isGeneric) {
					...
					if(!ReflectionUtils.IsNullOrDefault(defaultValue) && !(graph is uNodeStruct)) {
						if(defaultValue is UnityEngine.Object obj && obj != graph) {
							result += (m + vType + " " + name + " = " + ParseValue(defaultValue) + ";").AddFirst("\n", !string.IsNullOrEmpty(result));
						} else {
							result += (m + vType + " " + name + " = " + ParseValue(defaultValue) + ";").AddFirst("\n", !string.IsNullOrEmpty(result));
						}
					} else {
		...
		public class CData {
			...
			public string GenerateCode() {
				...
				if(obj.parameters != null && obj.parameters.Length > 0) {
					int index = 0;
					var parametersData = obj.parameters.Select(i => new MPData(i.name, ParseType(i.type), i.refKind));
				...
				if(obj.localVariable != null && obj.localVariable.Count > 0) {
					foreach(var vdata in obj.localVariable) {
						...
						if(vdata.type.isAssigned && vdata.type.targetType == MemberData.TargetType.Type && vdata.type.startType.IsValueType && vdata.value == null) {
							lv += (ParseType(vdata.type) + " " + GetVariableName(vdata) + ";").AddFirst("\n", !string.IsNullOrEmpty(lv));
							continue;
						}
						...
						lv += (ParseType(vdata.type) + " " + GetVariableName(vdata) +
		...
		public class PData {
			...
			public string GenerateCode() {
				...
				result += (m + ParseType(obj.ReturnType()) + " " + name + " " + p).AddFirst("\n", !string.IsNullOrEmpty(result));
				if(!string.IsNullOrEmpty(summary)) {
					result = "/// <summary>".AddLineInEnd() +
						"/// " + summary.Replace("\n", "\n" + "/// ").AddLineInEnd() +
						"/// </summary>" +
		...
		public class MData {
			...
			public string GenerateCode() {
				...
				if(name == "Awake" && graph is IIndependentGraph) {
				...
				codeList.Sort((x, y) => string.Compare(x.Key.ToString(), x.Key.ToString()));
				code = null;
		...
		public class GeneratedData {
			...
			public int GetGenerationID() {
				if(generationID == 0) {
					generationID = UnityEngine.Random.Range(1, int.MaxValue);
				}
				return generationID;
			}
			...
			private class GraphInformationToken {
				...
				public bool isStart => value.StartsWith("/*" + KEY_INFORMATION_HEAD);
				public bool isEnd => value.StartsWith("/*" + KEY_INFORMATION_TAIL);
