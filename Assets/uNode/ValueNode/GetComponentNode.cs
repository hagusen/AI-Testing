...
namespace MaxyGames.uNode.Nodes {
	...
	public class GetComponentNode : ValueNode {
		...
		[Hide, FieldConnection("Value"), Filter(typeof(Component), typeof(GameObject))]
				...
				if(t != null) {
					if(t.IsInterface || t.IsCastableTo(typeof(Component))) {
						if(t is RuntimeType runtimeType) {
							CodeGenerator.RegisterUsingNamespace("MaxyGames");//Register namespace to make sure Extensions work for GameObject or Component target type.
							if(CodeGenerator.generatePureScript) {
								switch(getComponentKind) {
									case GetComponentKind.GetComponent:
										return CodeGenerator.ParseValue(target).InvokeCode(
									...
									case GetComponentKind.GetComponentInChildren:
										return CodeGenerator.ParseValue(target).InvokeCode(
											...
											new[] { t }, 
											new[] { includeInactive.ParseValue() }
										).RemoveLast();
									case GetComponentKind.GetComponentInParent:
										return CodeGenerator.ParseValue(target).InvokeCode(
											nameof(uNodeHelper.GetGeneratedComponentInChildren), 
								...
								switch(getComponentKind) {
									case GetComponentKind.GetComponent:
										return CodeGenerator.ParseValue(target).InvokeCode(
											nameof(uNodeHelper.GetGeneratedComponent), 
											new[] { CodeGenerator.GetUniqueNameForComponent(runtimeType)
										}).RemoveLast();
									case GetComponentKind.GetComponentInChildren:
										return CodeGenerator.ParseValue(target).InvokeCode(
											nameof(uNodeHelper.GetGeneratedComponentInChildren), 
											new[] { CodeGenerator.GetUniqueNameForComponent(runtimeType), includeInactive.ParseValue() } 
										).RemoveLast();
									case GetComponentKind.GetComponentInParent:
										return CodeGenerator.ParseValue(target).InvokeCode(
											nameof(uNodeHelper.GetGeneratedComponentInChildren), 
							...
							switch(getComponentKind) {
								case GetComponentKind.GetComponent:
									return CodeGenerator.ParseValue(target).InvokeCode(nameof(Component.GetComponent), new[] { t }, null).RemoveLast();
								case GetComponentKind.GetComponentInChildren:
									return CodeGenerator.ParseValue(target).InvokeCode(nameof(Component.GetComponentInChildren), new[] { t }, new[] { includeInactive.ParseValue() }).RemoveLast();
								case GetComponentKind.GetComponentInParent:
									return CodeGenerator.ParseValue(target).InvokeCode(nameof(Component.GetComponentInParent), new[] { t }, new[] { includeInactive.ParseValue() }).RemoveLast();
							}
						}
					} else {
						throw new System.InvalidOperationException("The type is not supported to use GetComponent: " + t.FullName);
