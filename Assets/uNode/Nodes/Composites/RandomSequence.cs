...
namespace MaxyGames.uNode.Nodes {
	[NodeMenu("Composites", "RandomSequence", IsCoroutine = true)]
		...
		public override string GenerateCode() {
			...
			if(targetNodes.Count > 0) {
				...
				string RandomData = null;
				{
					...
					RandomData += CodeGenerator.GenerateForStatement("int " + index + " = " + (transitionCount - 1),
						index + " >= 0", "--" + index, contents).AddLineInFirst();
				}
				{
					...
					for(int i = 0; i < targetNodes.Count; i++) {
						...
						s += CodeGenerator.GenerateCondition("if", CodeGenerator.CompareNodeState(t.GetTargetNode(), false, false), failReturnVal);
