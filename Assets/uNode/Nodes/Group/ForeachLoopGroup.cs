...
namespace MaxyGames.uNode.Nodes {
	...
	public class ForeachLoopGroup : GroupNode {
		...
		public override string GenerateCode() {
			string ta = CodeGenerator.ParseValue((object)targetArray);
			if(!string.IsNullOrEmpty(ta)) {
				string data = null;
				string varName;
				if(CodeGenerator.NeedInstanceVariable(this)) {
					Variables[0].modifier.SetPrivate();
					varName = CodeGenerator.GenerateVariableName("foreach_item", this);
					CodeGenerator.AddVariable(Variables[0]);
					data += CodeGenerator.GetVariableName(Variables[0]) + " = " + varName + ";";
				} else {
					varName = CodeGenerator.AddVariable(Variables[0], false);
				}
				string result = "foreach(" + CodeGenerator.ParseType(targetArray.type.IsArray ? targetArray.type.GetElementType() : targetArray.type.GetGenericArguments()[0]) + " " + varName + " in " + ta + "){";
				if(!string.IsNullOrEmpty(data)) {
					result += ("\n" + data).AddTabAfterNewLine(1);
				}
				string tNode = CodeGenerator.GenerateFlowCode(nodeToExecute, this);
				if(!string.IsNullOrEmpty(tNode)) {
					result += ("\n" + tNode).AddTabAfterNewLine(1) + "\n";
				} else if(!string.IsNullOrEmpty(data)) {
					result += "\n";
				}
				return result + "}" + CodeGenerator.GetFinishCode(this, true, false, false).AddLineInFirst();
			}
			return null;
		}
	}
